...
func test_template_scenes_instantiate_without_errors() -> void:
	...
	for scene_path in scene_files:
		...
		add_child(instance)
		# Allow one frame for _ready() to complete (synchronous check)
		# Note: The scene tree will process this within the same frame context
...
func test_systems_and_grid_positioner_stack_work_together() -> void:
	...
	for combo in test_combinations:
		...
		var grid_stack_scene := load(combo["grid_stack"]) as PackedScene
		...
		# Allow initialization without awaiting (synchronous validation)
		# Both instances will be checked for proper tree membership below
