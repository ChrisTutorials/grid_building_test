...
func test_debug_enter_build_mode_failure() -> void:
	...
	(
		...
		. append_failure_message("BuildingSystem should be initialized")
		. is_not_null()
	)
	(
		assert_that(smithy_placeable)
		. append_failure_message("Smithy placeable should load successfully")
		. is_not_null()
	)
	...
	(
		...
		. append_failure_message("Indicator context should be available")
		. is_not_null()
	)
	(
		assert_that(indicator_context.has_manager())
		. append_failure_message("Indicator manager should be registered")
		. is_true()
	)
	...
	(
		...
		. append_failure_message("Indicator manager instance should exist")
		. is_not_null()
	)
	...
	(
		assert_that(building_issues)
		. append_failure_message(
			"Building system should have no runtime issues, but found: " + str(building_issues)
		)
		. is_empty()
	)
	...
	(
		assert_that(ready_check)
		. append_failure_message(
			"Building system should be ready to build. Issues: %s" % str(building_issues)
		)
		. is_true()
	)
	...
	(
		assert_that(setup_report.is_successful())
		. append_failure_message(
			"Enter build mode should succeed. Issues: " + str(setup_report.get_issues())
		)
		. is_true()
	)
