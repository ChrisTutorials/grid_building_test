...
func _assert_tile_coverage(
	bounds: Rect2, expected_tiles: Vector2i, case_name: String, tile_size: Vector2 = TILE_SIZE
) -> void:
...
func test_capsule_shape_validation(
	...
	var polygon: PackedVector2Array = GBGeometryMath.convert_shape_to_polygon(
		capsule_shape, transform
	)
...
## Test shape positioning and transformation at different world positions
@warning_ignore("unused_parameter")
func test_shape_positioning_validation(
	...
	if shape_type == TestShapeType.CAPSULE:
		...
		for p: Vector2 in shape_data["points"]:
