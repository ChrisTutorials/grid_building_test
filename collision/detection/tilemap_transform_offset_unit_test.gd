...
func test_tilemap_offset_affects_tile_overlap() -> void:
	...
	var offsets: Array[Vector2i] = CollisionGeometryUtils.compute_polygon_tile_offsets(world_points, TILE_SIZE, center_tile, tile_map.tile_set.tile_shape, tile_map)
	...
	var expected_core: Array[Vector2i] = [Vector2i(-1, -1), Vector2i(0, -1), Vector2i(1, -1),
	...
	var diag := _format_diagnostic(expected_core, offsets, missing, tile_map, world_center, center_tile)
...
func _get_missing_expected_tiles(expected: Array[Vector2i], offsets_set: Dictionary) -> Array[Vector2i]:
...
func _format_diagnostic(expected: Array[Vector2i], offsets: Array[Vector2i], missing: Array[Vector2i], map_layer: TileMapLayer, world_center: Vector2, center_tile: Vector2i) -> String:
	var s := "Expected tiles: %s\nActual offsets (%d): %s\nMissing tiles (%d): %s\nTileMap position: %s\nWorld center: %s\nComputed center_tile: %s" % [str(expected), offsets.size(), str(offsets), missing.size(), str(missing), str(map_layer.position), str(world_center), str(center_tile)]
	return s
