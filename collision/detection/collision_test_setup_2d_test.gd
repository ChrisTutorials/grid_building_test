...
func test_adjust_rect_to_testing_size() -> void:
	...
	var skew: Node2D = _create_rectangular_collision_object(
		"SkewRotationTestObject", Vector2(128, 128)
	)
	add_child(skew)
	...
	for setup in all_setups:
		...
		for rect_test in rect_tests:
			...
			var result_rect: Rect2 = setup._adjust_rect_to_testing_size(
				rect, setup.collision_object.global_transform
			)
