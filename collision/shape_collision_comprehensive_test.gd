...
func test_shape_bounds_validation(
	...
	test_parameters := [
		...
		},
		{
			...
			"radius": 14.0,
			"height": 60.0,
			"expected_bounds": Rect2(-14, -46, 28, 60)
		},
		{
			"name": "Large Capsule",
			"type": "capsule",
			"radius": 48.0,
			"height": 128.0,
			"expected_bounds": Rect2(-48, -112, 96, 128)
		},
		{
	...
	var expected = shape_data["expected_bounds"]
	var tolerance = 1.0

	(
		assert_float(bounds.position.x)
		.append_failure_message("%s left bound mismatch" % shape_data["name"])
		.is_equal_approx(expected.position.x, tolerance)
	)

	(
		assert_float(bounds.position.y)
		.append_failure_message("%s top bound mismatch" % shape_data["name"])
		.is_equal_approx(expected.position.y, tolerance)
	)

	(
		assert_float(bounds.size.x)
		.append_failure_message("%s width mismatch" % shape_data["name"])
		.is_equal_approx(expected.size.x, tolerance)
	)

	(
		assert_float(bounds.size.y)
		.append_failure_message("%s height mismatch" % shape_data["name"])
