...
func test_trapezoid_collision_detection_integration() -> void:
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] === STARTING INTEGRATION TEST ===")
	...
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Test object position: %s" % str(test_object.global_position))
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Positioner position: %s" % str(_targeting_state.positioner.global_position))
	...
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Owner shapes found: %d" % owner_shapes.size())

	var diag: PackedStringArray = PackedStringArray()
	diag.append("[TRAPEZOID_TRACE] Owner shapes found: %d" % owner_shapes.size())
	...
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Direct collision calculation tile offsets: %s" % str(tile_offsets))
	...
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Checking for expected missing tiles: %s" % str(expected_missing_tiles))
	GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Checking for expected present tiles: %s" % str(expected_present_tiles))
	...
	for tile: Vector2i in expected_present_tiles:
		...
		GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Expected tile %s present: %s" % [tile, is_present])

	for tile: Vector2i in expected_missing_tiles:
		var is_present: bool = tile_offsets.has(tile)
		GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] Expected missing tile %s present: %s" % [tile, is_present])
		if is_present:
			GBTestDiagnostics.log_verbose("[TRAPEZOID_TRACE] *** KEY FINDING: Tile %s is calculated but not appearing in runtime!" % tile)## Test the collision mapping integration specifically
func test_collision_mapper_integration() -> void:
	GBTestDiagnostics.log_verbose("[MAPPER_TRACE] === COLLISION MAPPER INTEGRATION TEST ===")
	...
	GBTestDiagnostics.log_verbose("[MAPPER_TRACE] Testing collision_mapper.map_collision_positions_to_rules")
	...
	GBTestDiagnostics.log_verbose("[MAPPER_TRACE] Position rules map size: %d" % position_rules_map.size())
	var positions: Array[Vector2i] = position_rules_map.keys()
	positions.sort()  # Sort for consistent output
	GBTestDiagnostics.log_verbose("[MAPPER_TRACE] Mapped positions: %s" % str(positions))
	...
	for pos: Vector2i in expected_extensions:
		...
		GBTestDiagnostics.log_verbose("[MAPPER_TRACE] Expected extension %s mapped: %s" % [pos, is_mapped])
		if not is_mapped:
			GBTestDiagnostics.log_verbose("[MAPPER_TRACE] *** ISSUE FOUND: Position %s not mapped by collision_mapper!" % pos)

## Test the full indicator generation integration
func test_full_indicator_generation_integration() -> void:
	GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] === FULL INDICATOR GENERATION INTEGRATION ===")
	...
	GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] Running IndicatorManager.try_setup")
	...
	if not report.is_successful():
		var diag1: PackedStringArray = PackedStringArray()
		diag1.append("[INTEGRATION_TRACE] Placement failed with issues: %s" % str(report.get_issues()))
		GBTestDiagnostics.log_verbose(diag1[0])
	...
	GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] Generated indicators count: %d" % indicators.size())
	...
	GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] Indicator positions: %s" % str(indicator_positions))
	...
	for pos: Vector2i in missing_positions:
		...
		GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] Position %s has indicator: %s" % [pos, has_indicator])

		if not has_indicator:
			GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] *** RUNTIME ISSUE CONFIRMED: Position %s missing from indicators!" % pos)
		else:
			GBTestDiagnostics.log_verbose("[INTEGRATION_TRACE] *** Position %s correctly generated!" % pos)

	# This test should help us identify exactly where the issue occurs
	var diag2: PackedStringArray = PackedStringArray()
	diag2.append("[INTEGRATION_TRACE] Indicator count check")
