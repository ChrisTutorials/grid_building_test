...
class PolygonTestFactory:
	static func create_square(
		size: float = 16.0, center: Vector2 = Vector2.ZERO
	) -> PackedVector2Array:
	...
	static func create_rectangle(
		width: float, height: float, center: Vector2 = Vector2.ZERO
	) -> PackedVector2Array:
	...
	static func create_triangle(
		size: float = 20.0, center: Vector2 = Vector2.ZERO
	) -> PackedVector2Array:
		return PackedVector2Array(
			[
	...
	static func create_u_shape(
		size: float = 32.0, center: Vector2 = Vector2.ZERO
	) -> PackedVector2Array:
	...
	static func create_concave_chevron(
		size: float = 24.0, center: Vector2 = Vector2.ZERO
	) -> PackedVector2Array:
		var s: float = size / 4.0
		return PackedVector2Array(
			[
...
class TileRectFactory:
	static func create_tile_rect(
		tile_x: int, tile_y: int, tile_size: Vector2 = DEFAULT_TILE_SIZE
	) -> Rect2:
...
func test_trapezoid_world_transform_and_tile_offsets() -> void:
	...
	var center_tile := Vector2i(
		int(positioner.global_position.x / 16.0), int(positioner.global_position.y / 16.0)
	)
	var offsets := CollisionGeometryUtils.compute_polygon_tile_offsets(
		world_points_array, tile_size, center_tile, TileSet.TILE_SHAPE_SQUARE
	)
	offsets.sort_custom(
...
func test_compute_polygon_tile_offsets(
	...
	test_parameters := [
		...
		[
			...
			Vector2(16, 16),
			1,
			"16x16 square at center of tile should cover 1 tile"
		],
		[
			"two_tile_rectangle",
			PackedVector2Array(
				[Vector2(-24, -8), Vector2(24, -8), Vector2(24, 8), Vector2(-24, 8)]
			),
			Vector2(32, 32),
			Vector2(16, 16),
			2,
			"48x16 rectangle should cover 2+ tiles horizontally"
		],
		[
			"four_tile_large_square",
			PackedVector2Array(
			...
			Vector2(16, 16),
			4,
			"32x32 square should cover 4 tiles"
		],
		...
		[
			...
			Vector2(16, 16),
			1,
			"Tiny 2x2 polygon should still register overlap"
		],
		[
			...
			Vector2(16, 16),
			1,
			"Polygon exactly on tile boundary should overlap"
		],
		[
			"cross_tile_boundary",
			PackedVector2Array([Vector2(-8, -8), Vector2(8, -8), Vector2(8, 8), Vector2(-8, 8)]),
			...
			Vector2(16, 16),
			4,
			"Polygon crossing tile boundaries should overlap multiple tiles"
		],
		# Triangle shapes
		[
			"triangle_single_tile",
			PackedVector2Array([Vector2(0, -10), Vector2(-8, 8), Vector2(8, 8)]),
			Vector2(20, 20),
			Vector2(16, 16),
			1,
			"Triangle within single tile"
		],
		[
			"triangle_multi_tile",
			PackedVector2Array([Vector2(0, -20), Vector2(-16, 16), Vector2(16, 16)]),
			Vector2(32, 32),
			Vector2(16, 16),
			2,
		...
		[
			"diamond_shape",
			PackedVector2Array([Vector2(0, -12), Vector2(12, 0), Vector2(0, 12), Vector2(-12, 0)]),
			Vector2(32, 32),
			Vector2(16, 16),
			1,
			"Diamond shape centered in tile"
		],
		[
			"trapezoid_from_runtime",
			PolygonTestFactory.create_trapezoid_from_runtime(),
			Vector2(440, 552),
			Vector2(16, 16),
			2,
			"Trapezoid from runtime should cover at least 2 tiles"
		],
		# Different tile sizes
		[
			"large_tile_small_polygon",
			PackedVector2Array([Vector2(-4, -4), Vector2(4, -4), Vector2(4, 4), Vector2(-4, 4)]),
			Vector2(40, 40),
			Vector2(40, 40),
			1,
			"Small polygon in large tile"
		],
		[
			"small_tile_large_polygon",
			PackedVector2Array(
				[Vector2(-16, -16), Vector2(16, -16), Vector2(16, 16), Vector2(-16, 16)]
			),
			Vector2(10, 10),
			Vector2(8, 8),
			9,
			"Large polygon covering many small tiles"
		],
		# Irregular polygons
		[
			"l_shape",
		...
		[
			...
			Vector2(40, 40),
			1,
			"Coordinates from failing mapper tests"
		],
	]
) -> void:
	...
	var center_tile := Vector2i(
		int(world_position.x / tile_size.x), int(world_position.y / tile_size.y)
	)

	# Act - compute tile offsets
	var offsets := CollisionGeometryUtils.compute_polygon_tile_offsets(
		world_points, tile_size, center_tile, TileSet.TILE_SHAPE_SQUARE
	)
...
func test_edge_cases_and_boundaries(
	...
	test_parameters := [
		...
		[
			...
			0,
			true,
			"Empty polygon should return empty offsets"
		],
		[
			"single_point",
			PackedVector2Array([Vector2(32, 32)]),
			Vector2(16, 16),
			Vector2i(2, 2),
			0,
			true,
			"Single point should return empty offsets"
		],
		[
			"two_points_line",
			PackedVector2Array([Vector2(32, 32), Vector2(48, 32)]),
			Vector2(16, 16),
			Vector2i(2, 2),
			0,
			true,
			"Line segment should return empty offsets"
		],
		[
			"collinear_points",
			...
			0,
			true,
			"Collinear points should return empty offsets"
		],
		...
		[
			...
			1,
			true,
			"Tile-aligned square should cover exactly 1 tile"
		],
		[
			"half_tile_offset",
			...
			4,
			true,
			"Half-tile offset should cover 4 tiles"
		],
		# Micro polygons - sized to meet the 5% area threshold (minimum 12.8 square units for 16x16 tile)
		[
			"micro_square",
			PackedVector2Array(
				[Vector2(17, 17), Vector2(25, 17), Vector2(25, 25), Vector2(17, 25)]
			),
			Vector2(16, 16),
			Vector2i(1, 1),
			1,
			true,
			"Micro square (8x8=64 units, 25% of tile) should register 1 tile"
		],
		[
			"sub_pixel_triangle",
			PackedVector2Array([Vector2(16.1, 16.1), Vector2(23.9, 16.1), Vector2(20, 23.9)]),
			Vector2(16, 16),
			Vector2i(1, 1),
			1,
			true,
			"Sub-pixel triangle with sufficient area should register 1 tile"
		],
		[
			"floating_precision_square",
			PackedVector2Array(
				[Vector2(16.1, 16.1), Vector2(23.9, 16.1), Vector2(23.9, 23.9), Vector2(16.1, 23.9)]
			),
			Vector2(16, 16),
			Vector2i(1, 1),
			1,
			true,
			"Floating point precision square with sufficient area"
		],
		# Large tile sizes vs small polygons - ensure meeting 5% threshold
		[
			"small_in_large_tile",
			PackedVector2Array(
				[Vector2(16, 16), Vector2(24, 16), Vector2(24, 24), Vector2(16, 24)]
			),
			Vector2(32, 32),
			Vector2i(0, 0),
			1,
			true,
			"Small polygon (8x8=64 units, 6.25% of 32x32 tile) should register"
		],
		...
		[
			...
			196,
			true,
			"Massive polygon (200x200 units) covers 14x14=196 tiles"
		],
		...
		[
			...
			PackedVector2Array(
				[
					Vector2(15.999, 15.999),
					Vector2(16.001, 15.999),
					Vector2(16.001, 16.001),
					Vector2(15.999, 16.001)
				]
			),
			Vector2(16, 16),
			Vector2i(1, 0),
			0,
			true,
			"Tiny floating point precision polygon (0.002x0.002 units) returns 0 tiles due to 5% area threshold"
		],
		# Very small polygons below 5% threshold - should return 0 tiles
		[
			"truly_micro_square",
			PackedVector2Array(
				[Vector2(17, 17), Vector2(19, 17), Vector2(19, 19), Vector2(17, 19)]
			),
			Vector2(16, 16),
			Vector2i(1, 1),
			0,
			true,
			"Truly micro square (2x2=4 units, 1.56% of tile) should return 0 tiles due to 5% threshold"
		],
	]
) -> void:
	# Act - compute tile offsets
...
func test_polygon_convexity(
	...
	test_parameters := [
		...
		[
			...
			[6, 20.0, Vector2.ZERO],
			true,
			"Regular hexagon should be convex"
		],
		[
			"diamond_convex",
			"create_diamond",
		...
		[
			...
			[24.0, Vector2.ZERO],
			true,
			"Chevron shape is actually convex"
		],
		[
			"star_concave",
			"create_star",
			[5, 20.0, 10.0, Vector2.ZERO],
			false,
			"Star shape should be non-convex"
		],
		...
		[
			...
			[],
			true,
			"Degenerate triangle (collinear) is considered convex"
		],
		[
			"self_intersecting",
			...
			[],
			false,
			"Self-intersecting polygon should not be convex"
		],
	]
) -> void:
	...
	match polygon_factory:
		...
		"create_rectangle":
			polygon = PolygonTestFactory.create_rectangle(
				factory_params[0], factory_params[1], factory_params[2]
			)
		"create_triangle":
		...
		"create_concave_chevron":
			polygon = PolygonTestFactory.create_concave_chevron(
				factory_params[0], factory_params[1]
			)
		"create_regular_polygon":
		...
		"create_diamond":
			polygon = PackedVector2Array(
				[
					Vector2(0, -factory_params[0]),
					Vector2(factory_params[0], 0),
					Vector2(0, factory_params[0]),
					Vector2(-factory_params[0], 0)
				]
			)
...
func test_collision_geometry_calculator_concave_tile_overlap() -> void:
	...
	var overlapped_tiles: Array[Vector2i] = CollisionGeometryCalculator.calculate_tile_overlap(
		...
		tile_size,
		tile_type,
		GodotTestFactory.create_empty_tile_map_layer(self),
		0.01,
		min_overlap_ratio
	)
...
func test_polygon_overlaps_rect(
	...
	test_parameters := [
		...
		[
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"16x16 square should overlap centered tile"
		],
		[
			"large_square",
			"create_square",
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"20x20 square should overlap centered tile"
		],
		[
			"centered_rectangle",
			"create_rectangle",
			[16.0, 10.0, Vector2.ZERO],
			Vector2.ZERO,
			DEFAULT_MIN_OVERLAP,
			true,
			"16x10 rectangle should overlap centered tile"
		],
		[
			"centered_triangle",
			"create_triangle",
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"Triangle should overlap centered tile"
		],
		# No overlap cases
		[
			"distant_square",
			...
			DEFAULT_MIN_OVERLAP,
			false,
			"Distant square should not overlap centered tile"
		],
		[
			"far_triangle",
			"create_triangle",
			...
			DEFAULT_MIN_OVERLAP,
			false,
			"Far triangle should not overlap centered tile"
		],
		...
		[
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"Small square should pass 5% overlap threshold"
		],
		[
			"small_square_strict",
			"create_square",
			...
			STRICT_MIN_OVERLAP,
			false,
			"Small square should fail 15% overlap threshold"
		],
		[
			"tiny_square_loose",
			"create_square",
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"Tiny square should pass 5% threshold"
		],
		[
			"tiny_square_strict",
			"create_square",
			...
			STRICT_MIN_OVERLAP,
			false,
			"Tiny square should fail 15% threshold"
		],
		...
		[
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"L-shape should overlap center tile"
		],
		[
			"u_shape_center_loose",
			"create_u_shape",
			[32.0, Vector2.ZERO],
			Vector2.ZERO,
			DEFAULT_MIN_OVERLAP,
			true,
			"U-shape center overlap with 5% threshold"
		],
		[
			"u_shape_center_strict",
			"create_u_shape",
			[32.0, Vector2.ZERO],
			Vector2.ZERO,
			STRICT_MIN_OVERLAP,
			true,
			"U-shape center overlap with 15% threshold"
		],
		[
			"u_shape_center_very_strict",
			"create_u_shape",
			[32.0, Vector2.ZERO],
			Vector2.ZERO,
			VERY_STRICT_MIN_OVERLAP,
			true,
			"U-shape center overlap with 25% threshold"
		],
		[
			"concave_chevron",
			"create_concave_chevron",
			...
			DEFAULT_MIN_OVERLAP,
			true,
			"Concave chevron should overlap center"
		],
		...
		[
			...
			DEFAULT_MIN_OVERLAP,
			false,
			"Empty polygon should not overlap"
		],
		[
			"single_point",
			"",
			...
			DEFAULT_MIN_OVERLAP,
			false,
			"Single point should not overlap"
		],
		[
			"line_segment",
			"",
			...
			DEFAULT_MIN_OVERLAP,
			false,
			"Line segment should not overlap"
		],
	]
) -> void:
...
func test_isometric_transformations(
	...
	test_parameters := [
		...
		[
			...
			[32.0, Vector2.ZERO],
			4,
			"30° skewed square should cover 4+ tiles"
		],
		[
			"skew_45_square",
			"skew",
			...
			[32.0, Vector2.ZERO],
			3,
			"45° skewed square should cover 3+ tiles (geometric analysis confirmed)"
		],
		[
			"skew_30_trapezoid",
			"skew",
			...
			[],
			2,
			"30° skewed trapezoid should cover 2+ tiles"
		],
		...
		[
			...
			[32.0, Vector2.ZERO],
			4,
			"30° rotated square should cover 4+ tiles"
		],
		[
			"rotate_45_square",
			"rotation",
			...
			[32.0, Vector2.ZERO],
			3,
			"45° rotated square should cover 3+ tiles (geometric analysis confirmed)"
		],
		[
			"rotate_30_trapezoid",
			"rotation",
			...
			[],
			2,
			"30° rotated trapezoid should cover 2+ tiles"
		],
		[
			"rotate_60_rectangle",
			"rotation",
			60.0,
			"create_rectangle",
			[40.0, 20.0, Vector2.ZERO],
			3,
			"60° rotated rectangle should cover 3+ tiles"
		],
		...
		[
			...
			[32.0, Vector2.ZERO],
			4,
			"30° combined transform square should cover 4+ tiles"
		],
		[
			"combined_45_square",
			"combined",
			...
			[32.0, Vector2.ZERO],
			4,
			"45° combined transform square should cover 4+ tiles"
		],
		[
			"combined_30_trapezoid",
			"combined",
			30.0,
			"create_trapezoid_from_runtime",
			[],
			3,
			"30° combined transform trapezoid should cover 3+ tiles"
		],
		...
		[
			...
			[24.0, Vector2.ZERO],
			2,
			"30° rotated triangle should cover 2+ tiles"
		],
		[
			"skew_30_l_shape",
			"skew",
			...
			[32.0, Vector2.ZERO],
			3,
			"30° skewed L-shape should cover 3+ tiles"
		],
	]
) -> void:
	...
	var _CalcScript := preload(
		"res://addons/grid_building/placement/manager/components/collision_geometry_calculator.gd"
	)
	...
	match polygon_factory:
		...
		"create_rectangle":
			base_polygon = PolygonTestFactory.create_rectangle(
				factory_params[0], factory_params[1], factory_params[2]
			)
		"create_triangle":
	...
	match transform_type:
		...
		"combined":
			var skew_transform := Transform2D(
				Vector2(1, tan(angle_rad)), Vector2(0, 1), Vector2.ZERO
			)
...
func test_isometric_diamond_single_tile_collision() -> void:
	...
	var diamond_polygon: PackedVector2Array = PackedVector2Array(
		[Vector2(-42.0, 0.0), Vector2(0.0, -24.0), Vector2(42.0, 0.0), Vector2(0.0, 24.0)]  # Left point  # Top point  # Right point  # Bottom point
	)
	...
	var overlapped_tiles: Array[Vector2i] = CollisionGeometryCalculator.calculate_tile_overlap(
		world_polygon, Vector2(tile_set.tile_size), tile_set.tile_shape, tile_map, 0.01, 0.05  # epsilon  # min_overlap_ratio (5%)
	)
