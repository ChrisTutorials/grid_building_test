...
func test_polygon_overlap_threshold(
	...
	test_parameters := [
		...
		[
			...
			0.05,
			true,
			"half-area triangle (0.5) vs 5%"
		],
		# 2) small triangle area=0.04 (base=1, height=0.08) -- below 5% but epsilon logic applies
		[
			[Vector2(0, 0), Vector2(1, 0), Vector2(0, 0.08)],
			0.05,
			true,
			"area=0.04 vs 5% (epsilon path)"
		],
		...
		[
			[Vector2(0, 0), Vector2(1, 0), Vector2(0, 0.025)],
			0.05,
			true,
			"area=0.0125 vs 5% (epsilon)"
		],
	...
	# Act
	var result: bool = CollisionGeometryCalculator.polygon_overlaps_rect(
		poly, rect, 0.01, min_overlap_ratio
	)
	...
	(
		...
		. append_failure_message(
			(
				"%s | clipped_area=%.6f min_ratio=%.3f expected=%s"
				% [description, clipped_area, min_overlap_ratio, str(expected)]
			)
		)
		. is_equal(expected)
	)
