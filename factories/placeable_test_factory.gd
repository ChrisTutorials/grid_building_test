...
static func create_test_placeable_with_rules(
	...
	placeable.placement_rules = PlacementRuleTestFactory.create_standard_placement_rules(
		include_tile_rule
	)
...
static func create_polygon_test_placeable(test_instance: Node) -> Placeable:
	...
	var polygon_obj := CollisionObjectTestFactory.create_polygon_test_object(
...
static func create_polygon_test_setup(test_instance: Node) -> Dictionary:
	...
	if (
		polygon_placeable.placement_rules != null
		and not polygon_placeable.placement_rules.is_empty()
	):
	...
	return {"rules": rules, "placeable": polygon_placeable}
...
static func create_basic_test_placeable(
	base_placeable: Placeable, display_name: String = "Basic Test Placeable"
) -> Placeable:
	var placeable: Placeable = Placeable.new()
	placeable.packed_scene = base_placeable.packed_scene
	placeable.display_name = display_name
...
static func create_minimal_test_placeable(
	display_name: String = "Minimal Test Placeable", include_rules: bool = false
) -> Placeable:
	var placeable: Placeable = Placeable.new()
	placeable.display_name = display_name
...
## @param include_tile_rule: Whether to include ValidPlacementTileRule
static func create_smithy_test_placeable(
	smithy_placeable: Placeable, include_tile_rule: bool = true
) -> Placeable:
	return create_test_placeable_with_rules(
		smithy_placeable, "Smithy Test Placeable", include_tile_rule
	)
...
static func validate_test_placeable(
	placeable: Placeable, context: String = "Test placeable"
) -> bool:
	if placeable == null:
