...
func test_create_node2d() -> void:
	...
	(
		assert_object(node) \
		. append_failure_message("create_node2d: node should not be null") \
		. is_not_null()
	)
	(
		assert_object(node.get_parent()) \
		. append_failure_message("create_node2d: node should be parented to test instance") \
		. is_equal(self)
	)
...
func test_create_node() -> void:
	...
	(
		assert_object(node.get_parent()) \
		. append_failure_message("create_node: node should be parented to test instance") \
		. is_equal(self)
	)
...
func test_create_canvas_item() -> void:
	...
	(
		assert_object(item) \
		. append_failure_message("create_canvas_item: item should not be null") \
		. is_not_null()
	)
	(
...
func test_create_tile_map_layer_with_grid() -> void:
	...
	(
		assert_object(layer) \
		. append_failure_message("create_tile_map_layer_with_grid: layer should not be null") \
		. is_not_null()
	)
	(
		assert_object(layer.get_parent()) \
		. append_failure_message(
			"create_tile_map_layer_with_grid: layer should be parented to test instance"
		) \
		. is_equal(self)
	)
	(
		...
		. append_failure_message(
			"create_tile_map_layer_with_grid: layer.tile_set should not be null"
		) \
		. is_not_null()
	)
	...
	(
		...
		. append_failure_message("Packed test tilemap should be 31x31") \
		. is_equal(Vector2(31, 31))
	)
...
func test_create_empty_tile_map_layer() -> void:
	...
	(
		assert_object(layer) \
		. append_failure_message("create_empty_tile_map_layer: layer should not be null") \
		. is_not_null()
	)
	(
		assert_object(layer.get_parent()) \
		. append_failure_message(
			"create_empty_tile_map_layer: layer should be parented to test instance"
		) \
		. is_equal(self)
	)
	(
...
func test_create_area2d_with_circle_shape() -> void:
	...
	(
		assert_that(circle_shape.radius) \
		. append_failure_message(
			(
...
func test_create_collision_polygon_default() -> void:
	...
	(
		assert_object(polygon) \
		. append_failure_message("CollisionPolygon2D should not be null") \
		. is_not_null()
	)
	(
		assert_object(polygon.get_parent()) \
		. append_failure_message(
			(
				"CollisionPolygon2D should be parented to test instance, parent: %s"
				% str(polygon.get_parent())
			)
		) \
		. is_equal(self)
	)
	(
		assert_object(polygon.polygon) \
		. append_failure_message("CollisionPolygon2D should have polygon points assigned") \
		. is_not_null()
	)
	(
		assert_int(polygon.polygon.size()) \
		. append_failure_message(
			(
				"Default collision polygon should have %d points (triangle), got: %d points"
				% [TRIANGLE_VERTEX_COUNT, polygon.polygon.size()]
			)
		) \
...
func test_create_collision_polygon_custom() -> void:
	...
	(
		assert_object(polygon) \
		. append_failure_message("Custom CollisionPolygon2D should not be null") \
		. is_not_null()
	)
	(
		assert_object(polygon.get_parent()) \
		. append_failure_message(
			(
				"Custom CollisionPolygon2D should be parented to test instance, parent: %s"
				% str(polygon.get_parent())
			)
		) \
		. is_equal(self)
	...
	(
		...
		. append_failure_message(
			(
				"Custom collision polygon should have %d points (rectangle), got: %d points, input: %s"
				% [RECTANGLE_VERTEX_COUNT, polygon.polygon.size(), str(custom_points)]
			)
		) \
	...
	(
		...
		. append_failure_message(
			(
				"Custom collision polygon points mismatch - Expected: %s, Actual: %s"
				% [str(custom_points), str(polygon.polygon)]
			)
		) \
...
func test_create_object_with_circle_shape() -> void:
	...
	(
		assert_object(obj) \
		. append_failure_message(
			"create_object_with_circle_shape: returned object should not be null"
		) \
		. is_not_null()
	)
	(
		...
		. append_failure_message(
			"create_object_with_circle_shape: object should be parented to test instance"
		) \
		. is_equal(self)
	)
	...
	(
		...
		. append_failure_message(
			"create_object_with_circle_shape: StaticBody2D collision_layer mismatch"
		) \
		. is_equal(1)
	)
	...
	(
		...
		. append_failure_message(
			"create_object_with_circle_shape: collision_shape.shape should be CircleShape2D"
		) \
...
func test_create_parent_with_body_and_polygon() -> void:
	...
	(
		...
		. append_failure_message(
			"create_parent_with_body_and_polygon: parent should be parented to test instance"
		) \
		. is_equal(self)
	)
	(
		...
		. append_failure_message(
			"create_parent_with_body_and_polygon: expected 2 children on parent"
		) \
		. is_equal(2)
	)
...
func test_create_rectangle_shape() -> void:
	...
	(
		assert_object(shape) \
		. append_failure_message("create_rectangle_shape: shape should not be null") \
		. is_not_null()
	)
	(
		assert_object(shape) \
	...
	(
		...
		. append_failure_message("create_rectangle_shape: shape size mismatch") \
		. is_equal(Vector2(50, 60))
	)
...
func test_create_circle_shape() -> void:
	...
	(
		assert_object(shape) \
		. append_failure_message("create_circle_shape: shape should not be null") \
		. is_not_null()
	)
	(
		assert_object(shape) \
	...
	(
		...
		. append_failure_message("create_circle_shape: radius mismatch") \
		. is_equal(25.0)
	)
...
func test_create_rule_check_indicator() -> void:
	...
	(
		...
		. append_failure_message("create_rule_check_indicator: indicator should not be null") \
		. is_not_null()
	)
	(
	...
	(
		assert_object(indicator.shape) \
		. append_failure_message(
			"create_rule_check_indicator: indicator.shape should be RectangleShape2D"
		) \
		. is_instanceof(RectangleShape2D)
	)
	...
	(
		assert_that(rect_shape.size) \
		. append_failure_message("create_rule_check_indicator: rect_shape.size mismatch") \
...
func _assert_static_body_valid(body: StaticBody2D, context: String) -> void:
	...
	(
		assert_object(body) \
		. append_failure_message("StaticBody2D should not be null for context: %s" % context) \
		. is_not_null()
	)

	(
		assert_object(body.get_parent()) \
		. append_failure_message(
			(
				"StaticBody2D should be parented to test instance for context: %s, parent: %s"
				% [context, str(body.get_parent())]
			)
		) \
		. is_same(self)
	)
	...
	(
		assert_int(body.get_child_count()) \
		. append_failure_message(
			(
				"StaticBody2D should have %d children for context: %s, actual count: %d"
				% [EXPECTED_PARENT_CHILDREN, context, body.get_child_count()]
			)
		) \
...
func _assert_area2d_valid(area: Area2D, context: String) -> void:
	...
	(
		assert_object(area) \
		. append_failure_message("Area2D should not be null for context: %s" % context) \
		. is_not_null()
	)

	(
		assert_object(area.get_parent()) \
		. append_failure_message(
			(
				"Area2D should be parented to test instance for context: %s, parent: %s"
				% [context, str(area.get_parent())]
			)
		) \
		. is_same(self)
	)
	...
	(
		assert_int(area.get_child_count()) \
		. append_failure_message(
			(
				"Area2D should have %d children for context: %s, actual count: %d"
				% [EXPECTED_PARENT_CHILDREN, context, area.get_child_count()]
			)
		) \
		. is_equal(EXPECTED_PARENT_CHILDREN)
	)
	...
	(
		assert_int(area.collision_layer) \
		. append_failure_message(
			(
				"Area2D collision layer should be %d for context: %s, actual: %d"
				% [DEFAULT_COLLISION_LAYER, context, area.collision_layer]
			)
		) \
...
func _get_rect_shape_from_body(body: StaticBody2D) -> RectangleShape2D:
	...
	(
		...
		. append_failure_message(
			(
				"StaticBody2D should have CollisionShape2D as first child, got: %s"
				% str(collision_shape)
			)
		) \
		. is_not_null()
	...
	(
		...
		. append_failure_message(
			(
				"CollisionShape2D should have RectangleShape2D shape, got: %s"
