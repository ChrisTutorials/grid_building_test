...
static func create_collision_rule_with_settings(
	apply_mask: int, collision_mask: int, pass_on_collision: bool = true
) -> CollisionsCheckRule:
...
## @param apply_mask: Objects mask to apply rule to
static func create_valid_tile_rule(
...
static func create_default_collision_rule() -> CollisionsCheckRule:
	return create_collision_rule_with_settings(
		DEFAULT_COLLISION_LAYER, DEFAULT_COLLISION_MASK, false
	)
...
static func create_rules_from_templates(
	base_collision_rule: CollisionsCheckRule = null, base_tile_rule: TileCheckRule = null
) -> Array[PlacementRule]:
	var rules: Array[PlacementRule] = []
