...
func test_trapezoid_debug() -> void:
	...
	for expected_tile : Vector2i in expected_tiles:
		var tile_rect: Rect2 = Rect2(
			Vector2(expected_tile.x * tile_size.x, expected_tile.y * tile_size.y), 
			tile_size
		)
		...
		details += "  Overlap area: " + str(area) + " / " + str(tile_area) + " = " + str(overlap_ratio) + " (" + str(overlap_ratio * 100) + "% )\n"
		
		# Test with different thresholds
		var thresholds: Array[float] = [0.01, 0.05, 0.12]
		for threshold : float in thresholds:
			var passes: bool = overlap_ratio >= threshold
			details += "    Threshold " + str(threshold * 100) + "%: " + ("PASS" if passes else "FAIL") + "\n"
	...
	assert_int(actual_offsets.size()).append_failure_message(
