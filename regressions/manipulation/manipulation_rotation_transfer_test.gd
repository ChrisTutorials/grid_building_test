...
func before_test() -> void:
	...
	assert_object(_manipulation_system).is_not_null().append_failure_message(
		"ManipulationSystem should be initialized in AllSystemsTestEnvironment"
	)
	assert_object(_indicator_manager).is_not_null().append_failure_message(
		"IndicatorManager should be initialized in AllSystemsTestEnvironment"
	)
	assert_object(_manipulation_parent).is_not_null().append_failure_message(
		"ManipulationParent should be initialized in AllSystemsTestEnvironment"
	)
	assert_object(_container).is_not_null().append_failure_message(
...
func test_indicator_manager_is_child_of_manipulation_parent() -> void:
	...
	assert_object(_indicator_manager).is_not_null().append_failure_message(
		"IndicatorManager should exist in container"
	)
	
	# Assert: ManipulationParent exists
	assert_object(_manipulation_parent).is_not_null().append_failure_message(
		"ManipulationParent should exist in container"
	)
	
	# Assert: IndicatorManager is child of ManipulationParent
	var manager_parent: Node = _indicator_manager.get_parent()
	assert_object(manager_parent).is_equal(_manipulation_parent).append_failure_message(
...
func test_manipulation_parent_starts_at_zero_rotation() -> void:
	...
	assert_float(_manipulation_parent.rotation).is_equal(0.0).append_failure_message(
		"ManipulationParent should start at rotation=0, actual: %f" % _manipulation_parent.rotation
	)
...
func test_rotation_transferred_after_indicator_generation() -> void:
	...
	assert_object(manipulation_data).is_not_null().append_failure_message(
	...
	assert_array(indicators).is_not_empty().append_failure_message(
		"Indicators should have been generated. Count: %d" % indicators.size()
	)
	
	# CRITICAL ASSERTION 4: Indicators should be children of IndicatorManager
	for i in indicators.size():
		var indicator: RuleCheckIndicator = indicators[i]
		var indicator_parent: Node = indicator.get_parent()
		assert_object(indicator_parent).is_equal(_indicator_manager).append_failure_message(
			"Indicator[%d] should be child of IndicatorManager. Actual parent: %s" % [
				i, str(indicator_parent.name) if indicator_parent != null else "null"
			]
		)
...
func test_indicator_count_consistent_across_rotations() -> void:
	...
	assert_int(count_90deg).is_equal(count_0deg).append_failure_message(
...
func test_rotation_transferred_to_parent_after_indicator_generation() -> void:
	...
	var move_result: ManipulationData = _manipulation_system.try_move(_smithy.root)
	assert_object(move_result).is_not_null().append_failure_message(
		"Move should return valid ManipulationData"
	)
	assert_int(move_result.status).is_equal(GBEnums.Status.STARTED).append_failure_message(
	...
	print("[TEST] Checking ManipulationParent (instance_id=%d) rotation=%f scale=%s" % [_manipulation_parent.get_instance_id(), _manipulation_parent.rotation, str(_manipulation_parent.scale)])
	assert_float(_manipulation_parent.rotation).is_equal_approx(original_rotation, 0.01).append_failure_message(
		...
		"Expected: %f, Actual: %f" % [original_rotation, _manipulation_parent.rotation]
	)
