...
func test_all_systems_specific_present() -> void:
	assert_that(test_env.manipulation_system).append_failure_message("ManipulationSystem should be present").is_not_null()
	assert_that(test_env.target_highlighter).append_failure_message("TargetHighlighter should be present").is_not_null()
...
func test_grid_targeting_system_positioner_connection() -> void:
	...
	assert_that(test_env.grid_targeting_system).append_failure_message("GridTargetingSystem should be properly initialized").is_not_null()
...
func test_environment_no_issues() -> void:
	...
	assert_array(all_issues).append_failure_message("Environment should have no issues: " + str(all_issues)).is_empty()
...
func test_building_system_can_enter_build_mode() -> void:
	...
	if placeable != null:
		...
		assert_that(result.is_successful()).append_failure_message(
			"BuildingSystem should be able to enter build mode with test placeable"
		).is_true()
	else:
...
func test_scene_node_structure() -> void:
	...
	assert_that(test_env.get_node_or_null("World/GridPositioner2D")).append_failure_message("GridPositioner2D should exist at expected path").is_not_null()
	assert_that(test_env.get_node_or_null("World/GridPositioner2D/ManipulationParent")).append_failure_message("ManipulationParent should exist").is_not_null()
	assert_that(
		test_env.get_node_or_null("World/GridPositioner2D/ManipulationParent/IndicatorManager")
	).append_failure_message("IndicatorManager should exist at expected path").is_not_null()
...
func test_placement_rules_available() -> void:
	...
	if container != null:
		...
		assert_that(placement_rules).append_failure_message("Placement rules collection should exist").is_not_null()
