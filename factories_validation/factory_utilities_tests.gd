...
func after_test() -> void:
	...
	if test_duration > _max_test_duration_ms:
		push_warning("Test took %d ms, exceeds %d ms threshold" % [test_duration, _max_test_duration_ms])
...
func _assert_object_not_null(obj: Object, context: String) -> void:
	...
	assert_object(obj).append_failure_message(
		"%s should not be null" % context
	).is_not_null()

func _assert_performance_threshold(actual_ms: int, operation: String, threshold: int = PERFORMANCE_THRESHOLD_MS) -> void:
	"""Assert performance is within acceptable threshold"""
	assert_int(actual_ms).append_failure_message(
...
func _assert_memory_usage_change(initial: int, final: int, operation: String) -> void:
	...
	assert_int(memory_diff).append_failure_message(
...
func test_test_node2d_factory_robustness() -> void:
	...
	assert_str(node_class).append_failure_message(
		"Created object should be Node2D type, got: %s" % node_class
	).is_equal("Node2D")
	...
	assert_object(test_node.get_parent()).append_failure_message(
		"Node2D should be properly parented for cleanup"
	).is_not_null()
...
func test_test_tilemap_factory_validation() -> void:
	...
	if test_tilemap:
		...
		assert_str(tilemap_class).append_failure_message(
			"Should be TileMapLayer type, got: %s" % tilemap_class
		).is_equal("TileMapLayer")

		# Configuration validation
		assert_object(test_tilemap.tile_set).append_failure_message(
			"TileMapLayer should have valid tile_set assigned"
		).is_not_null()
		...
		_assert_performance_threshold(perf_end - perf_start, "Setting %d tiles" % TILE_OPERATION_COUNT)
...
func test_composition_container_factory_robustness() -> void:
	...
	assert_object(contexts.owner).append_failure_message(
		"Container should have owner context configured"
	).is_not_null()
...
func test_factory_performance_and_cleanup() -> void:
	...
	assert_int(created_objects.size()).append_failure_message(
		"Should create all requested objects"
...
func test_factory_stress_and_recovery() -> void:
	...
	assert_float(success_rate).append_failure_message(
