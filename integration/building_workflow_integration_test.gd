...
## Tests the full system integration across BuildingSystem, IndicatorManager, and ManipulationSystem
...
func before_test():
	...
	injector_system = UnifiedTestFactory.create_test_injector(self, _container)
	...
	var _indicator_manager = UnifiedTestFactory.create_test_indicator_manager(self, _container)
...
func after_test():
	...
	var placement_ctx := _container.get_contexts().indicator if _container else null
...
func test_placement_validation_workflow():
	...
	var generated_instance := building_system.instance_preview(test_elipse_placeable_skew_rotation)
	auto_free(generated_instance)
	positioner.global_position = Vector2.ZERO

	# Explicitly setup placement rules including a CollisionCheckRule so occupied tiles invalidate placement
	var indicator_manager = _container.get_contexts().indicator.get_manager()
	...
	var setup_report : PlacementSetupReport = indicator_manager.try_setup(rules, validation_params)
	...
	assert_bool(validation_result.is_successful()).append_failure_message("Expected success by has issues %s" % str(validation_result.get_issues())).is_true()
	...
	var setup_ok : PlacementSetupReport = indicator_manager.try_setup(rules, validation_params)
	assert_bool(setup_ok.is_successful()).append_failure_message("Failed to re-setup placement rules for second preview").is_true()

	var result : ValidationResults = indicator_manager.validate_placement()
	assert_bool(result.is_successful()).append_failure_message("Expected invalid placement on occupied tile. Failure reasons %s" % str(result.get_issues())).is_false()
	_assert_no_orphans()
...
func test_indicator_generation_on_enter_build_mode_with_smithy():
	...
	var indicator_manager := _container.get_contexts().indicator.get_manager()
	assert_object(indicator_manager).append_failure_message("IndicatorManager not available").is_not_null()
	...
	var setup_ok := indicator_manager.try_setup(rules, validation_params)
	assert_object(setup_ok).append_failure_message("IndicatorManager.try_setup returned null").is_not_null()
	assert_bool(setup_ok.is_successful()).append_failure_message("IndicatorManager.try_setup failed for smithy").is_true()
	...
	for ind in indicators:
		if ind is RuleCheckIndicator:
			...
			# Verify indicator is parented to the manipulation parent (runtime behavior)
			assert_object(ind.get_parent()).append_failure_message("Indicator should be parented to manipulation parent").is_equal(_container.get_states().manipulation.parent)
...
func test_rotation_regression_indicators_maintain_independent_rotation() -> void:
	...
	var indicator_manager : IndicatorManager = _container.get_contexts().indicator.get_manager()
	assert_object(indicator_manager).append_failure_message("IndicatorManager should exist").is_not_null()
	...
	# Assert: Verify preview has rotated but indicators maintain their original rotation
	var expected_preview_rotation : float = initial_preview_rotation + rotation_amount
	...
	).is_equal_approx(expected_preview_rotation, 0.001)

	# Indicators should NOT rotate with preview since they're parented to IndicatorManager
	for i in range(indicators.size()):
		var indicator : RuleCheckIndicator = indicators[i]
		var original_rotation : float = initial_indicator_rotations[i]
		assert_float(indicator.global_rotation_degrees).append_failure_message(
			"Indicator %d should maintain its original rotation (not rotate with preview)" % i
		).is_equal_approx(original_rotation, 0.001)
...
func test_placeable_switch_regression_indicators_freed_properly() -> void:
	...
	var indicator_manager : IndicatorManager = _container.get_contexts().indicator.get_manager()
	assert_object(indicator_manager).append_failure_message("IndicatorManager should exist").is_not_null()
	var indicators1 : Array[RuleCheckIndicator] = indicator_manager.get_indicators()
	...
	var colliding_before_switch : Array[RuleCheckIndicator] = indicator_manager.get_colliding_indicators()
	...
	var indicators2 : Array[RuleCheckIndicator] = indicator_manager.get_indicators()
	...
	var colliding_after_switch : Array[RuleCheckIndicator] = indicator_manager.get_colliding_indicators()
