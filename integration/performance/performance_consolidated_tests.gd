...
var ALL_SYSTEMS_ENV_SCENE: PackedScene = GBTestConstants.get_environment_scene(
...
func test_collision_mapping_performance() -> void:
	...
	for test_setup: Variant in test_setups:
		if test_setup != null:
			var offsets: Dictionary = collision_mapper.get_tile_offsets_for_test_collisions(
				test_setup
			)
...
func test_concurrent_operations_performance() -> void:
	...
	var setups: Array[CollisionTestSetup2D] = (
		CollisionTestSetup2D.create_test_setups_from_test_node(test_area, targeting_state)
	)
	var test_setup: CollisionTestSetup2D = setups[0] if setups.size() > 0 else null
