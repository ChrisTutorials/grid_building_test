...
func before_test() -> void:
	...
	test_hierarchy = ALL_SYSTEMS_ENV_SCENE.instantiate()
...
func test_collision_mapper_basic_functionality() -> void:
	...
	var test_scene: PackedScene = load("res://godot/test/grid_building_test/scenes/objects/test_rect_15_tiles.tscn")
	var collision_object: Node2D = test_scene.instantiate()
	test_hierarchy.positioner.add_child(collision_object)
	auto_free(collision_object)
	
	# Create CollisionTestSetup2D using DRY pattern
...
func test_collision_performance_large_tilemap() -> void:
	...
	var rect_shape: RectangleShape2D = RectangleShape2D.new()
	rect_shape.size = Vector2(128, 128)
	var area: Area2D = Area2D.new()
	var collision_shape: CollisionShape2D = CollisionShape2D.new()
	collision_shape.shape = rect_shape
	area.add_child(collision_shape)
