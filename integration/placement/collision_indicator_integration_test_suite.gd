...
func before_test() -> void:
	...
	assert_array(issues).is_empty() \
		.append_failure_message("Test environment must be properly configured: " + str(issues))
...
func test_collision_mapper_basic_functionality() -> void:
	...
	assert_that(collision_mapper).is_not_null() \
		.append_failure_message("CollisionMapper must be available from IndicatorManager")
	...
	assert_that(test_scene).is_not_null() \
		.append_failure_message("Test rectangle scene must load successfully")

	var collision_object: Node2D = test_scene.instantiate() as Node2D
	assert_that(collision_object).is_not_null() \
		.append_failure_message("Test rectangle object must instantiate successfully")
	...
	assert_that(targeting_state).is_not_null() \
		.append_failure_message("Targeting state must be available from container")

	# Act
	var setups: Array[CollisionTestSetup2D] = CollisionTestSetup2D.create_test_setups_from_test_node(collision_object, targeting_state)
	var test_setup: CollisionTestSetup2D = setups[0] if setups.size() > 0 else null

	# Assert
	assert_that(test_setup).is_not_null() \
		.append_failure_message("CollisionTestSetup2D must be created successfully")

	var offsets: Dictionary[Vector2i, Array] = collision_mapper.get_tile_offsets_for_test_collisions(test_setup)
	assert_dict(offsets).is_not_empty() \
		.append_failure_message("Collision mapping must produce tile offsets")
...
func test_collision_mapper_multiple_shapes() -> void:
	...
	# Assert
	assert_that(test_setup).is_not_null() \
		.append_failure_message("CollisionTestSetup2D must be created for multi-shape object")

	var offsets: Dictionary[Vector2i, Array] = collision_mapper.get_tile_offsets_for_test_collisions(test_setup)
	assert_dict(offsets).is_not_empty() \
		.append_failure_message("Collision mapping must produce tile offsets for multi-shape object")
...
func test_indicator_manager_setup_basic() -> void:
	...
	assert_that(indicator_manager).is_not_null() \
		.append_failure_message("IndicatorManager must be available")

	var manipulation_parent: Node2D = test_hierarchy.manipulation_parent
	assert_that(manipulation_parent).is_not_null() \
		.append_failure_message("ManipulationParent must be available")
...
func test_collision_performance_large_tilemap() -> void:
	...
	var collision_mapper: CollisionMapper = test_hierarchy.indicator_manager.get_collision_mapper()
	assert_that(collision_mapper).is_not_null() \
		.append_failure_message("CollisionMapper must be available")

	# Create a larger collision object
	var rect_shape: RectangleShape2D = RectangleShape2D.new()
	rect_shape.size = Vector2(LARGE_TILEMAP_SIZE * GBTestConstants.DEFAULT_TILE_SIZE.x, LARGE_TILEMAP_SIZE * GBTestConstants.DEFAULT_TILE_SIZE.y)
	var area: Area2D = Area2D.new()
	var collision_shape: CollisionShape2D = CollisionShape2D.new()
	collision_shape.shape = rect_shape
	area.add_child(collision_shape)
	...
	var test_setup: CollisionTestSetup2D = setups[0] if setups.size() > 0 else null
	assert_that(test_setup).is_not_null() \
		.append_failure_message("CollisionTestSetup2D must be created for large tilemap test")
	...
	assert_int(processing_time).is_less_equal(int(GBTestConstants.TEST_TIMEOUT_MS / 10.0)) \
		.append_failure_message("Large tilemap collision processing should complete within reasonable time")
	assert_dict(offsets).is_not_empty() \
		.append_failure_message("Collision mapping must produce offsets for large tilemap")
...
func test_collision_performance_multiple_objects() -> void:
	...
	# Assert
	var processing_time: int = end_time - start_time
	assert_int(processing_time).is_less_equal(int(GBTestConstants.TEST_TIMEOUT_MS / 5.0)) \
		.append_failure_message("Multiple object collision processing should complete within reasonable time")
	assert_int(all_offsets.size()).is_equal(PERFORMANCE_TEST_ITERATIONS) \
		.append_failure_message("All collision objects must produce offset results")
...
func test_systems_integration_environment_validation() -> void:
	...
	var setups: Array[CollisionTestSetup2D] = CollisionTestSetup2D.create_test_setups_from_test_node(test_static_body, targeting_state)
	var test_setup: CollisionTestSetup2D = setups[0] if setups.size() > 0 else null
	...
	var rules: Array[TileCheckRule] = [TileCheckRule.new()]
	var report: IndicatorSetupReport = test_hierarchy.indicator_manager.setup_indicators(test_static_body, rules)
	assert_that(report).is_not_null()
...
func test_systems_environment_consolidation_validation() -> void:
	...
	var collision_result: Dictionary = test_hierarchy.indicator_manager.get_collision_mapper().get_tile_offsets_for_test_collisions(test_setup)
	assert_dict(collision_result).is_not_empty()
	
	# Test indicator setup (would require indicator_manager factory)
	var rules: Array[TileCheckRule] = [TileCheckRule.new()]
	var indicator_result: IndicatorSetupReport = test_hierarchy.indicator_manager.setup_indicators(test_static_body, rules)
	...
	push_warning("CONSOLIDATION BENEFIT: AllSystemsTestEnvironment provides single access point for all systems")
	push_warning("ELIMINATES NEED: Separate factory calls for collision_mapper, indicator_manager, tile_map, positioner")
