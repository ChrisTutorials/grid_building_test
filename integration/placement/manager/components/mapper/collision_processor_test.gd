...
func _create_collision_test_setup(collision_obj: CollisionObject2D) -> IndicatorCollisionTestSetup:
	return IndicatorCollisionTestSetup.new(collision_obj, _test_env.tile_size)

## Helper method to verify basic collision processing results (DRY principle)
func _verify_collision_result(result: Dictionary, expected_min_tiles: int, expected_max_tiles: int, test_description: String):
...
func test_collision_processor_shapes_square_tiles(
	...
	# Assert
	_verify_collision_result(result, expected_min_tiles, expected_max_tiles, "%s shape" % shape_type)
...
func test_collision_processor_polygon_square_tiles():
	...
	var result = _processor.get_tile_offsets_for_collision(polygon_node, null, _test_env.top_down_map, _test_env.positioner)
	...
	assert_that(result.size()).append_failure_message("Expected 1-16 tiles for 32x32 polygon, got %d tiles" % result.size()).is_between(1, 16)
...
func test_collision_processor_isometric_tiles():
	...
	# Act
	var result = _processor.get_tile_offsets_for_collision(collision_obj, test_data, _test_env.isometric_map, _test_env.positioner)
...
func test_collision_processor_cache_invalidation():
	...
	var result1 = _processor.get_tile_offsets_for_collision(collision_obj, test_data, _test_env.top_down_map, _test_env.positioner)
	...
	var result2 = _processor.get_tile_offsets_for_collision(collision_obj, test_data, _test_env.top_down_map, _test_env.positioner)
...
func test_calculate_tile_range_shapes(
	...
	# Act
	var result = _processor.calculate_tile_range(shape, bounds, _test_env.top_down_map, _test_env.tile_size, shape_transform)
...
func test_process_shape_offsets_rectangle():
	var collision_obj = CollisionObjectTestFactory.create_static_body_with_rect(self, Vector2(32, 32))
	collision_obj.position = _test_env.test_position
	
	var test_data = _create_collision_test_setup(collision_obj)
	var rect_test_setup = test_data.rect_collision_test_setups[0] as RectCollisionTestingSetup
	
	var shape_epsilon = 0.1
...
func test_compute_shape_tile_offsets_rectangle():
	...
	var start_tile = Vector2i(50, 40)
	var end_exclusive = Vector2i(55, 45)
	var shape_epsilon = 0.1
	
	# Act
	var result = _processor.compute_shape_tile_offsets(shape, shape_transform, _test_env.top_down_map, _test_env.tile_size, shape_epsilon, start_tile, end_exclusive, _test_env.center_tile, shape_polygon)
