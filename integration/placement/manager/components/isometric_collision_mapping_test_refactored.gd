...
const TestDebugHelpers = preload("res://test/grid_building_test/integration/placement/manager/components/debug_helpers/test_debug_helpers.gd")
...
func test_isometric_small_diamond_single_tile():
...
func test_isometric_square_building_single_tile():
...
func test_isometric_medium_diamond_precision():
...
func _get_tile_position_count_for_polygon(polygon: PackedVector2Array[Node2D]) -> int:
	...
	var tile_positions_dict: Dictionary[Vector2i, Array[Node2D]] = _collision_mapper.get_collision_tile_positions_with_mask(collision_objects, building.collision_layer)
	var tile_positions: Array[Node2D][Vector2i] = tile_positions_dict.keys()
	
	return tile_positions.size()

## DRY helper for parameterized testing of multiple building shapes
@warning_ignore("unused_parameter")
func test_isometric_building_shapes(shape_name: String, polygon: PackedVector2Array[Node2D], expected_tiles: int, description: String):
...
func test_isometric_building_shapes_parameters() -> Array[Node2D]:
	return [
		["Small Diamond", PackedVector2Array[Node2D]([Vector2(-42, 0), Vector2(0, -24), Vector2(42, 0), Vector2(0, 24)]), 1, "84x48px diamond should fit in single 90x50 tile"],
		["Square Building", PackedVector2Array[Node2D]([Vector2(-40, -20), Vector2(40, -20), Vector2(40, 20), Vector2(-40, 20)]), 1, "80x40px square should fit in single 90x50 tile"],
		["Medium Diamond", PackedVector2Array[Node2D]([Vector2(-48, -16), Vector2(0, -44), Vector2(48, -16), Vector2(0, 12)]), 1, "96x56px diamond should fit in single 90x50 tile with proper calculation"]
	]
