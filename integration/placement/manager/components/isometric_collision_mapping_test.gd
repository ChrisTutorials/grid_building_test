...
func test_isometric_collision_mapping_precision() -> void:
	...
	for building_data: Dictionary in test_buildings:
		var building_name: String = building_data.name
		var polygon: PackedVector2Array = building_data.polygon
		var expected_count: int = building_data.expected_tiles
		var description: String = building_data.description
		
		total_expected += expected_count
		
		# Create test building
		var building: StaticBody2D = auto_free(UnifiedTestFactory.create_static_body_with_polygon(self, polygon))
		add_child(building)
		
		# Test collision mapping
		var actual_count: int = get_tile_position_count(building, _tile_map)
	...
	for building_name: String in results.keys():
		var result: Dictionary = results[building_name]
...
func get_tile_position_count(building: StaticBody2D, tilemap: TileMapLayer) -> int:
	...
	var targeting_state: GridTargetingState = auto_free(GridTargetingState.new(GBOwnerContext.new()))
	...
	var debug_settings: GBDebugSettings = auto_free(GBDebugSettings.new())
	var logger: GBLogger = auto_free(GBLogger.new(debug_settings))
	var collision_mapper: CollisionMapper = auto_free(CollisionMapper.new(targeting_state, logger))
	
	var test_setup: Variant = TestFactory.create_test_indicator_collision_setup(self, building)
	collision_mapper.collision_object_test_setups[building] = test_setup
	
	# Calculate tile positions
	var collision_objects: Array[Node2D] = [building]
	var tile_positions: Dictionary = collision_mapper.get_collision_tile_positions_with_mask(collision_objects, building.collision_layer)
...
func test_isometric_padding_fix() -> void:
	...
	var mill_polygon: PackedVector2Array = PackedVector2Array([Vector2(-48, -16), Vector2(0, -44), Vector2(48, -16), Vector2(0, 12)])
	var building: StaticBody2D = auto_free(UnifiedTestFactory.create_static_body_with_polygon(self, mill_polygon))
	add_child(building)
	
	var tile_count: int = get_tile_position_count(building, _tile_map)
