...
func test_target_modulate_clears_on_target_null() -> void:
	...
	assert_that(highlighter.current_target).append_failure_message(
		"Highlighter's current_target should be null after targeting_state.clear()"
	).is_null()
	assert_that(target).append_failure_message(
		"Original target reference should still be valid"
	).is_not_null()
