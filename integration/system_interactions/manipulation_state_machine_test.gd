...
func test_cancel_with_valid_move_data_returns_all_commands() -> void:
	...
	(
		...
		.append_failure_message("Should clear data") \
		.is_true()
	)
	(
	...
	(
		...
		.append_failure_message("Should clear targeting") \
		.is_true()
	)
