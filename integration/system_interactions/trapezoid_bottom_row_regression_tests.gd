...
func before_test() -> void:
	_test_env = EnvironmentTestFactory.create_all_systems_env(self, GBTestConstants.ALL_SYSTEMS_ENV)
...
func test_trapezoid_bottom_row_coverage() -> void:
	...
	var tile_positions_dict: Dictionary[Vector2i, Array] = (
		_collision_mapper
		. get_tile_offsets_for_collision_polygon(collision_polygon, _test_env.tile_map_layer)
	)
