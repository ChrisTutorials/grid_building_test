...
const TRAPEZOID_POLYGON := [Vector2(-32, 12), Vector2(-16, -12), Vector2(17, -12), Vector2(32, 12)]
...
func test_collision_shape_tile_coverage_with_various_shape_types(
	...
	test_parameters := [
		[
			"rectangle",
			{"size": STANDARD_SHAPE_SIZE, "position": ORIGIN_POSITION},
			RECTANGLE_EXPECTED_TILES
		],
		[
			"trapezoid",
			{"polygon": TRAPEZOID_POLYGON, "position": ORIGIN_POSITION},
			TRAPEZOID_EXPECTED_TILES
		],
		["circle", {"radius": CIRCLE_RADIUS, "position": ORIGIN_POSITION}, CIRCLE_EXPECTED_TILES],
		[
			"rectangle_offset",
			{"size": MEDIUM_SHAPE_SIZE, "position": OFFSET_POSITION},
			OFFSET_RECTANGLE_EXPECTED_TILES
		]
	]
) -> void:
	...
	var tile_offsets: Dictionary[Vector2i, Array] = (
		collision_mapper
...
func test_collision_mapper_positioning_edge_cases_handle_problematic_positions(
	...
	var tile_offsets: Dictionary[Vector2i, Array] = (
		collision_mapper
		. get_tile_offsets_for_test_collisions(CollisionTestSetup2D.new(test_object, shape_size))
	)
...
func test_complex_polygon_shapes_handle_edge_cases_from_debug_tests() -> void:
	...
	for polygon_data in complex_polygons:
		...
		var tile_offsets: Dictionary[Vector2i, Array] = (
			collision_mapper.get_tile_offsets_for_test_collisions(indicator_test_setup)
		)
...
func test_collision_mapper_transform_consistency_across_different_transforms() -> void:
	...
	var shape_data: Dictionary[String, Variant] = {
		"size": STANDARD_SHAPE_SIZE, "position": ORIGIN_POSITION
	}
...
func test_rules_and_collision_integration() -> void:
	...
	var collision_tiles: Dictionary[Vector2i, Array] = (
		configured_collision_mapper.get_collision_tile_positions_with_mask(test_objects, 1)
	)
