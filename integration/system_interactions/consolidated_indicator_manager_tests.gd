...
const CollisionObjectTestFactoryGd = preload(
	"res://test/grid_building_test/factories/collision_object_test_factory.gd"
)
...
func test_collision_mapper_shape_positioning() -> void:
	...
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [test_object], Vector2(32, 32), 16
	)
	...
	for pos: Vector2 in test_positions:
		...
		var collision_tiles: Dictionary = (
			...
			. get_collision_tiles_for_objects(collision_mapper, [test_object])
		)
...
func test_shape_positioner_movement_consistency() -> void:
	var test_object: StaticBody2D = CollisionObjectTestFactory.create_static_body_with_circle(
		self, 16
	)
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [test_object], Vector2(32, 32), 16
	...
	var start_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
		collision_mapper, [test_object]
	)

	test_object.global_position = end_pos
	var end_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
		collision_mapper, [test_object]
	)
...
func test_collision_mapper_trapezoid_regression() -> void:
	...
	var trapezoid_object: StaticBody2D = CollisionObjectTestFactory.create_static_body_with_polygon(
		self, trapezoid_points
	)
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [trapezoid_object], Vector2(48, 16), 16
	)
	...
	var collision_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
		collision_mapper, [trapezoid_object]
	)
...
func test_polygon_indicator_runtime_parity() -> void:
	...
	var polygon_object: StaticBody2D = CollisionObjectTestFactory.create_static_body_with_polygon(
		self, polygon_points
	)
...
func test_real_world_indicator_scenarios() -> void:
	...
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [small_building], Vector2(16, 16), 16
	)

	small_building.global_position = Vector2(0, 0)
	var small_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
	...
	var large_building: StaticBody2D = CollisionObjectTestFactory.create_static_body_with_rect(
		self, Vector2(64, 64)
	)
	collision_mapper = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [large_building], Vector2(64, 64), 32
	)

	large_building.global_position = Vector2(0, 0)
	var large_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
...
func test_component_integration_workflow() -> void:
	...
	var complex_object: StaticBody2D = CollisionObjectTestFactory.create_complex_collision_object(
		self, Vector2(24, 24), 12.0, Vector2(20, 20)
	)

	# Set up collision mapper
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [complex_object], Vector2(44, 44), 24
	)
...
func test_placement_component_error_handling() -> void:
	...
	var dummy_object: StaticBody2D = CollisionObjectTestFactory.create_static_body_with_rect(
		self, Vector2(16, 16)
	)
	var collision_mapper: Object = CollisionObjectTestFactory.setup_collision_mapper_with_objects(
		self, env, [dummy_object], Vector2(16, 16), 16
	)

	# Test with empty array (should return empty result)
	var null_tiles: Dictionary = CollisionObjectTestFactory.get_collision_tiles_for_objects(
		collision_mapper, []
	)
	(
	...
	var invalid_result: PlacementReport = indicator_manager.try_setup(
