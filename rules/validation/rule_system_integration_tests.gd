...
func create_all_rules() -> Array[Object]:
	var rules: Array[Object] = [
		TileCheckRule.new(), CollisionsCheckRule.new(), WithinTilemapBoundsRule.new()
	]
	for rule in rules:
		auto_free(rule)
	return rules
...
func test_within_bounds_rule_edge_cases() -> void:
	...
	var edge_positions: Array[Vector2i] = [
		Vector2i(0, 0), Vector2i(-1, 0), Vector2i(0, -1), Vector2i(1, 0), Vector2i(0, 1)  # Top-left corner  # Just outside left  # Just outside top  # Just outside right  # Just outside bottom
	]
