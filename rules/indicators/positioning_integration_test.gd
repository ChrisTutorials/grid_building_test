...
func test_collision_mapping_produces_relative_offsets() -> void:
	...
	GBTestDiagnostics.log_verbose("Environment setup successful - collision mapper available at correct position")
	GBTestDiagnostics.log_verbose("The fix in collision_processor.gd should prevent absolute coordinates from being returned")
	GBTestDiagnostics.log_verbose("Instead of center_tile = collision_object.position, now uses center_tile = positioner.position")
	GBTestDiagnostics.log_verbose("This ensures offsets are calculated relative to where IndicatorFactory expects them")
	...
	assert_that(test_env.positioner.global_position).append_failure_message("Expected positioner at correct coordinates. Context: %s" % "\n".join(diag)).is_equal(Vector2(456.0, 552.0))
