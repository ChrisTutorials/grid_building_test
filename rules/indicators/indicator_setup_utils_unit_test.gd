...
func test_gather_collision_shapes_parameterized() -> void:
	for test_data: Dictionary in TEST_SCENE_DATA:
		...
		var result: Dictionary[Node2D, Array] = IndicatorSetupUtils.gather_collision_shapes(
			test_object
		)
...
func test_execute_indicator_setup_produces_zero_indicators_despite_collision_shapes() -> void:
	...
	var collision_shapes: Dictionary[Node2D, Array] = IndicatorSetupUtils.gather_collision_shapes(
		smithy_obj
	)
	...
	var test_setups: Dictionary[Node2D, CollisionTestSetup2D] = (
		IndicatorSetupUtils.build_collision_test_setups(collision_shapes, DEFAULT_TILE_SIZE)
	)
	(
...
func _validate_collision_mapping(
	...
	var collision_results: Dictionary[Vector2i, Array] = (
		collision_mapper.get_collision_tile_positions_with_mask(test_objects, mask)
	)
