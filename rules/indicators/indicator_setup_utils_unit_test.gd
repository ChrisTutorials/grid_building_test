...
const TEST_RECT_SIZE_SMALL := Vector2(GBTestConstants.DEFAULT_TILE_PIXEL * 2, GBTestConstants.DEFAULT_TILE_PIXEL * 2)
const TEST_RECT_SIZE_LARGE := Vector2(GBTestConstants.DEFAULT_TILE_PIXEL * 4, GBTestConstants.DEFAULT_TILE_PIXEL * 4)
...
func test_validate_indicator_positions_correct_positioning() -> void:
	...
	assert_that(result.size_mismatch)\
		.append_failure_message("Expected no size mismatch")\
		.is_false()
...
func test_validate_indicator_positions_size_mismatch() -> void:
	...
	assert_that(result.expected_count)\
		...
		.is_equal(2)
	assert_that(result.actual_count)\
		.append_failure_message("Actual count should be 1")\
		.is_equal(1)
...
func test_build_collision_test_setups_collision_polygon_owner() -> void:
	...
	assert_that(result[collision_owner])\
		...
		.is_null()  # CollisionPolygon2D gets null setup
...
func test_validate_setup_preconditions_null_test_object() -> void:
	...
	assert_that(result)\
		...
		.is_not_empty()
	assert_that(result.has("Test object is null or invalid"))\
		.is_true()
...
func _load_test_scene(scene_path: String, position: Vector2 = TEST_POSITION_CENTER) -> Node2D:
	var scene: PackedScene = load(scene_path) as PackedScene
	assert_object(scene)\
		.append_failure_message("Failed to load scene: " + scene_path)\
		.is_not_null()
...
func _run_indicator_setup(test_object: Node2D, rules: Array[TileCheckRule], expected_success: bool = true) -> IndicatorSetupUtils.SetupResult:
	...
	var indicator_parent := _create_indicator_parent()
	var setup_result: IndicatorSetupUtils.SetupResult = IndicatorSetupUtils.execute_indicator_setup(
		test_object, rules, collision_mapper, _indicator_template, indicator_parent, _targeting_state
	)
	if expected_success:
...
func test_multiple_rule_validation_setup() -> void:
	...
	assert_int(rules.size())\
		.append_failure_message("Expected 2 rules")\
		.is_equal(2)
