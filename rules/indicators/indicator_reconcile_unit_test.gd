...
func test_reconcile_reuses_existing_indicator_and_updates_rules() -> void:
    ...
    assert_that(existing_indicators.size() == 1).append_failure_message("Expected factory to create 1 existing indicator").is_true()
    var indicator1: RuleCheckIndicator = existing_indicators[0]
    assert_that(is_instance_valid(indicator1)).append_failure_message("Existing indicator should be valid").is_true()
    ...
    assert_that(new_indicators.size() == 1).append_failure_message("Expected factory to create 1 new indicator").is_true()
    ...
    assert_that(reconciled.size() == 1).append_failure_message("Expected 1 reconciled indicator").is_true()
    ...
    assert_that(rules_after.size() == 1).append_failure_message("Reused indicator should have exactly 1 rule after reconcile").is_true()
    assert_that(rules_after[0].resource_name == "B").append_failure_message("Reused indicator's rule should be rule_b").is_true()
