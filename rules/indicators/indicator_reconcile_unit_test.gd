...
func _create_env_and_service() -> Dictionary[String, Variant]:
...
func test_reconcile_reuses_existing_indicator_and_updates_rules() -> void:
    var ctx: Dictionary[String, Variant] = _create_env_and_service()
    ...
    var existing_indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators(pos_map_1, GBTestConstants.TEST_INDICATOR_TD_PLATFORMER, parent, ctx["container"].get_states().targeting, test_object)
    ...
    var new_indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators(pos_map_2, GBTestConstants.TEST_INDICATOR_TD_PLATFORMER, parent, ctx["container"].get_states().targeting, test_object)
    ...
    var reconciled: Array[RuleCheckIndicator] = service._reconcile_indicators(new_indicators)
    ...
    var rules_after: Array[TileCheckRule] = indicator2.get_rules()
