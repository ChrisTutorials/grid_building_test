...
func before_test() -> void:
  ...
  .append_failure_message("Failed to load indicator template").is_not_null()
	assert_that(_tile_map).append_failure_message("TileMap should be created").is_not_null()
	assert_that(_tile_map.tile_set)
  .append_failure_message("TileSet not properly assigned").is_not_null()
...
func test_indicator_positioning_regression_800_pixel_offset() -> void:
	...
	# Log detailed positioning data for analysis (use local diag)
	var diag: PackedStringArray = PackedStringArray()
	diag.append("=== POSITIONING REGRESSION TEST ===")
	diag.append("Distance: %.1f pixels" % distance)
	diag.append("Offset vector: %s" % [indicator_pos - expected_pos])

	assert_vector(expected_pos).append_failure_message("Expected position for indicator\n%s" % "\n".join(diag)).is_equal(indicator_pos)
 ...
 assert_that(distance).append_failure_message( "REGRESSION DETECTED: Indicator positioned at (%s), expected near (%s). " % [indicator_pos, expected_pos] + "Distance is %.1f pixels. The 800+ pixel offset regression means indicators appear far from expected positions." % distance ) # DEBUG TEST to investigate what position data comes from collision system func test_debug_collision_position_mapping() -> void: # This test verifies that realistic collision offsets don't create massive pixel displacements # Set up a realistic scenario matching the runtime environment _test_object.global_position = Vector2(456.0, 552.0) # From runtime analysis _positioner.global_position = Vector2(456.0, 552.0) # Use a realistic offset that matches what we see in working tests # From CollisionMapper tests, we see offsets like (-2, -1), (0, 0), (1, 1), etc. # These should create small displacements in the range of 16-32 pixels var realistic_offset := Vector2i(-2, 1) # Should create ~32-pixel displacement # Test with the realistic offset to ensure normal behavior var position_rules_map: Dictionary[Vector2i, Array] = {} position_rules_map[realistic_offset] = [] var indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators( position_rules_map, _indicator_template, _parent_node, _targeting_state, _test_object ) assert_that(indicators.size()).is_equal(1) var indicator: RuleCheckIndicator = indicators[0] var expected_pos := Vector2(456.0, 552.0) var actual_pos := indicator.global_position var distance := actual_pos.distance_to(expected_pos) var diag: PackedStringArray = PackedStringArray() diag.append("=== COLLISION POSITION MAPPING DEBUG ===") diag.append("Realistic offset: %s" % [realistic_offset]) diag.append("Expected position: %s" % [expected_pos]) diag.append("Actual position: %s" % [actual_pos]) diag.append("Distance: %.1f pixels" % distance) diag.append("Local position offset: %s" % [actual_pos - expected_pos]) # With realistic offsets, we should get reasonable pixel distances # (-2, 1) offset means 2 tiles left, 1 tile down = (-32, 16) pixels approximately var expected_distance_range_min := 30.0 # Allow some tolerance for positioning var expected_distance_range_max := 50.0 if distance > 100.0: diag.append("REGRESSION DETECTED: Large offset detected - positioning system creating wrong tile positions") diag.append("Tile offset %s creates %.1f pixel displacement" % [realistic_offset, distance]) fail("Realistic offset created unexpectedly large displacement: %.1f pixels\n%s" % [distance, "\n".join(diag)]) else: diag.append("POSITIONING WORKING: Realistic offset creates reasonable displacement") # Verify the offset creates displacement in the expected range assert_that(distance).is_greater(expected_distance_range_min).is_less(expected_distance_range_max).append_failure_message( "Realistic offset should create displacement in range %.1f-%.1f pixels, got %.1f pixels" % [expected_distance_range_min, expected_distance_range_max, distance] ) # Test with a small positive offset to ensure normal behavior still works var small_offset := Vector2i(1, 0) # Should create ~16 pixel offset var small_position_rules_map: Dictionary[Vector2i, Array] = {} small_position_rules_map[small_offset] = [] var small_indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators( small_position_rules_map, _indicator_template, _parent_node, _targeting_state, _test_object ) assert_that(small_indicators.size()).is_equal(1) var small_indicator := small_indicators[0] var small_distance := small_indicator.global_position.distance_to(expected_pos) diag.append("Small offset test: %s creates %.1f pixel distance" % [small_offset, small_distance]) # Small offsets should create reasonable distances (~16 pixels for 1 tile) assert_that(small_distance).is_greater(10.0).is_less(30.0).append_failure_message( "Small offset should create reasonable distance, got %.1f pixels" % small_distance ).is_less(LARGE_DISTANCE_THRESHOLD)