...
func test_tile_to_global_position_logic() -> void:
	...
	var indicators: Array = IndicatorFactory.generate_indicators(
		position_rules_map,
		TestIndicatorScene,
		_positioner, # Parent node for the indicators
		_targeting_state,
		_positioner # Test object is the positioner itself for simplicity
	)

	# Assert
	assert_int(indicators.size()).append_failure_message("IndicatorFactory should generate 1 indicator for 1 offset").is_equal(1)
	var indicator: Node2D = indicators[0]

	# Re-calculate expected position manually to verify logic
	var calculated_positioner_tile: Vector2i = _tile_map.local_to_map(_tile_map.to_local(_positioner.global_position))
	...
	assert_vector(actual_global_position).append_failure_message(failure_message).is_equal_approx(expected_global_position, Vector2(0.1, 0.1))
