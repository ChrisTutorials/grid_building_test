	...
	for indicator in indicators:
		...
		if name_parts.size() >= 2:
			...
			if offset_parts.size() >= 2:
				...
				assert_that(indicator.global_position).append_failure_message( \
...
func test_null_targeting_state_components_handled_gracefully() -> void:
	...
	assert_that(indicator.global_position).is_equal(Vector2(0, 0)) \
		.append_failure_message("Indicator with null targeting components should be at origin (0,0)")
