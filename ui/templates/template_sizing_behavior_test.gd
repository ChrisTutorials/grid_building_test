...
func test_placeable_template_size_flags() -> void:
	...
	assert_object(entry_node)\
		.is_not_null()\
		.append_failure_message("Template should be a Control")
	...
	assert_float(actual_width)\
		.append_failure_message("Template width %.1f should stretch beyond minimum (>100px)" % actual_width)\
		.is_greater(100.0)

	# Check height remains fixed
	var actual_height: float = entry_node.size.y
	assert_float(actual_height)\
		.append_failure_message("Template height %.1f should be exactly %d pixels" % [actual_height, EXPECTED_PLACEABLE_HEIGHT])\
...
func test_sequence_templates_stretch_to_column_width() -> void:
	...
	assert_object(template)\
		.is_not_null()\
		.append_failure_message("Sequence template should be a Control")
...
func test_height_consistency_during_variant_cycling() -> void:
	...
	assert_float(post_cycle_width)\
		.is_equal(initial_width)\
		.append_failure_message("Template width should remain %.1f pixels after cycling, now %.1f" % [initial_width, post_cycle_width])
...
func test_mixed_content_consistent_sizing() -> void:
	...
	for i in range(children.size()):
		...
		assert_object(template)\
			.is_not_null()\
			.append_failure_message("Mixed content template %d should be a Control" % i)
		...
		var expected_height: float = EXPECTED_PLACEABLE_HEIGHT if template is PlaceableView else EXPECTED_SEQUENCE_HEIGHT
		assert_float(actual_height)\
	...
	for i in range(1, expected_widths.size()):
		...
		assert_float(width_difference)\
			...
			.is_less(100.0)  # Increased tolerance since PlaceableView and PlaceableListEntry may differ
...
func test_size_flags_configuration() -> void:
	...
	assert_that(sequence_size_flags == Control.SIZE_EXPAND_FILL or sequence_size_flags == Control.SIZE_EXPAND)\
