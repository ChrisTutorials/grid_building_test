...
func test_generate_indicators_positions_correctly() -> void:
	...
	assert_that(indicators.size()).append_failure_message("[DEBUG] test_generate_indicators_positions_correctly: Positioner=%s TileSize=%s Indicators=%s" % [str(_positioner.global_position), str(TILE_SIZE), str(indicator_positions)]).is_equal(indicators.size())
...
func test_positioner_position_affects_indicator_positions() -> void:
	...
	assert_that(origin_indicator_pos).append_failure_message("[DEBUG] test_positioner_position_affects_indicator_positions: origin=%s offset=%s origin_indicator=%s offset_indicator=%s" % [str(Vector2.ZERO), str(Vector2(100,100)), str(origin_indicator_pos), str(offset_indicator_pos)]).is_not_null()
