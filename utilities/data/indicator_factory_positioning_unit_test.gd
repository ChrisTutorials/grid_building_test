...
# Temporarily disabled due to missing scene resources during testing
# This test depends on scene files that may need to be re-imported
# Original test_indicators_match_premade_object_collision_shapes() commented out
...
func before_test() -> void:
	...
	_indicator_template = load(
		"res://addons/grid_building/placement/rule_check_indicator/rule_check_indicator.tscn"
	)
...
func test_coordinate_transformation_pipeline() -> void:
	...
	var positioner_tile: Vector2i = _tile_map.local_to_map(
	...
	var distance_from_positioner: float = expected_global_pos.distance_to(
...
func test_generate_indicators_positions_correctly() -> void:
	...
	var indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators(
		position_rules_map, _indicator_template, _parent_node, _targeting_state, _positioner  # Pass positioner as test_object
	)
...
func test_positioner_position_affects_indicator_positions() -> void:
	...
	# Generate indicators at origin
	var indicators_at_origin: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators(
		position_rules_map_origin, _indicator_template, _parent_node, _targeting_state, _positioner  # Pass positioner as test_object
	)
	var origin_indicator_pos: Vector2 = (
		indicators_at_origin[0].global_position if indicators_at_origin.size() > 0 else Vector2.ZERO
	)
	...
	var offset_indicator_pos: Vector2 = (
		indicators_at_offset[0].global_position if indicators_at_offset.size() > 0 else Vector2.ZERO
	)
...
func test_indicators_use_global_positioning() -> void:
	...
	# Generate indicators
	var indicators: Array[RuleCheckIndicator] = IndicatorFactory.generate_indicators(
		position_rules_map, _indicator_template, _parent_node, _targeting_state, _positioner  # Pass positioner as test_object for consistency
	)
	...
	(
		...
		. append_failure_message(
			(
				...
				% [
					str(_parent_node.position),
					str(_parent_node.rotation),
					str(_parent_node.scale),
					str(expected_global_pos),
					str(actual_global_pos),
					position_error
				]
			)
		) \
...
func _verify_indicators_distributed_on_grid(
	indicators: Array[RuleCheckIndicator], expected_positions: Array[Vector2i]
) -> void:
