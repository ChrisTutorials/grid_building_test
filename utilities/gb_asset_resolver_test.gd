...
const TEST_ASSETS_FOLDER: String = "res://test/grid_building_test/resources/placeable/"
...
func test_load_placeables_from_folder() -> void:
	...
	var result: GBAssetResolver.LoadResult = GBAssetResolver.load_placeables_with_result(
		TEST_ASSETS_FOLDER
	)
	...
	(
		assert_str(summary)
		. append_failure_message("Summary should include asset count")
		. contains("Loaded:")
	)
	(
		assert_str(summary)
		. append_failure_message("Summary should include error count")
		. contains("Errors:")
	)
	(
		assert_str(summary)
		. append_failure_message("Summary should include warning count")
		. contains("Warnings:")
	)


func test_backward_compatibility_with_placeable_loader() -> void:
	# Test that GBAssetResolver.load_placeables produces similar results to
	# PlaceableLoader.get_placeables
	...
	if new_result.is_successful():
		(
			assert_int(new_result.assets.size())
		...
		for asset: Variant in new_result.assets:
			(
				assert_object(asset)
				. is_not_null()
				. append_failure_message("All loaded assets should be valid")
			)
...
func test_simple_placeable_loading_compatibility() -> void:
	...
	(
		assert_int(new_simple_result.size())
		. is_equal(old_result.size())
		. append_failure_message(
