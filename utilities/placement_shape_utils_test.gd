...
class_name PlacementShapeUtilsTest
...
func test_grow_rect2_to_increment():
	for pos in test_positions:
		for size in range(-100, 100, 3):
			...
			var rect_result : Rect2 = PlacementShapeUtils.grow_rect2_to_increment(start_rect, tile_size)
...
func test_grow_rect2_to_square():
	for pos in test_positions:
		for i in range(0,25,1):
			...
			var rect_result = PlacementShapeUtils.grow_rect2_to_square(start_rect)
