...
var test_positions = [Vector2(0, 0), Vector2(-100, 50), Vector2(2000, 50000)]
...
func test_grow_rect2_to_increment():
	for pos in test_positions:
		for size in range(-100, 100, 3):
			...
			(
				assert_bool(abs_size_result.x >= abs_size.x)
				. append_failure_message(
					"Size X: " + str(abs_size_result.x) + " >= " + str(abs_size.x)
				)
				. is_true()
			)
			(
				assert_bool(abs_size_result.y >= abs_size.y)
				. append_failure_message(
					"Size Y: " + str(abs_size_result.y) + " >= " + str(abs_size.y)
				)
				. is_true()
			)
			var max_resize = abs_size + tile_size
			(
				assert_bool(abs_size_result.x <= max_resize.x)
				. append_failure_message(
					"Size X: " + str(abs_size_result.x) + " <= " + str(max_resize.x)
				)
				. is_true()
			)
			(
				assert_bool(abs_size_result.y <= max_resize.y)
				. append_failure_message(
					"Size Y: " + str(abs_size_result.y) + " <= " + str(max_resize.y)
				)
				. is_true()
			)
