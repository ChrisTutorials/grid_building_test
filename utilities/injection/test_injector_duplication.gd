...
func test_automatic_container_duplication_in_scene() -> void:
	...
	assert_object(injector).is_not_null().append_failure_message(
		"Environment should have GBTestInjectorSystem"
	)
	...
	assert_object(scene_container).is_not_null().append_failure_message(
		"Injector should have a composition_container after scene load"
	)
	...
	assert_int(scene_id).is_not_equal(original_id).append_failure_message(
		"Container should be duplicated from resource during scene load. Original ID: %d, Scene ID: %d" % [original_id, scene_id]
	)
	
	# Verify the duplicated container has the same structure
	assert_object(scene_container.config).is_not_null().append_failure_message(
		"Duplicated container should have config"
	)
	assert_object(scene_container.config.settings).is_not_null().append_failure_message(
		"Duplicated container should have settings"
	)
...
func test_static_duplicate_helper() -> void:
	...
	assert_that(duplicated).is_not_null() \
		.append_failure_message("duplicate_container should return a container")
