...
func test_injector_system_instantiation() -> void:
 ...
 assert_object(injector)
 	...
 	.append_failure_message("Object assertion failed")
 	.is_not_null()
 	.append_failure_message( "Should find GBTestInjectorSystem in environment" ) # Check if composition_container is set var container: GBCompositionContainer = injector.composition_container assert_object(container)
 	.append_failure_message("Object assertion failed")
 	.is_not_null()
 	.append_failure_message( "Injector should have composition_container after scene load. Injector: %s, Container: %s" % [ injector, container ] ) # Verify container has a logger (basic validation) var logger: GBLogger = container.get_logger() assert_object(logger)
 	.append_failure_message("Object assertion failed")
 	.is_not_null()
 	.append_failure_message( "Container should have a logger" ) ## Test that container was duplicated during scene load (test isolation) func test_container_duplicated_during_scene_load() -> void: # Load the original container resource directly var original_resource_path: String = GBTestConstants.TEST_PATH_COMPOSITION_CONTAINER_TEST_RESOURCE var original_container: GBCompositionContainer = load(original_resource_path) var original_id: int = original_container.get_instance_id() # Get the container from the injector in the environment var injector_node: Node = env.injector assert_object(injector_node)
 	.append_failure_message("Object assertion failed")
 	.is_not_null()
 	.append_failure_message( "Should find GBInjectorSystem node in environment" ) # Use get() to access the composition_container property from the script var scene_container: GBCompositionContainer = injector_node.get("composition_container") assert_object(scene_container)
 	.append_failure_message("Object assertion failed")
 	.is_not_null()
 	.append_failure_message( "Environment injector should have a container. Script: %s" % (injector_node.get_script().resource_path if injector_node.get_script() else "none") ) var scene_id: int = scene_container.get_instance_id() # Verify they're different instances (duplication occurred) assert_int(scene_id)
 	.append_failure_message("Integer assertion failed")
 	.is_not_equal(original_id)
 	.append_failure_message( "Scene container should be duplicated from resource. Original ID: %d, Scene ID: %d" % [original_id, scene_id] )
 	.is_not_null()