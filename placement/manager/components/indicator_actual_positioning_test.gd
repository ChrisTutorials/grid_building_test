...
func test_actual_indicator_positioning():
	...
	test_rule.initialize_with_params(
		"test_rule", [], [Vector2i(0, 0), Vector2i(1, 1), Vector2i(2, 2)]
	)
	...
	var indicators = placement_manager.create_indicators(
		[collision_object], [test_rule], placement_manager
	)
	...
	for i in range(min(3, indicators.size())):
		...
		print(
			(
				"Indicator %d: global_position = %s (expected %s)"
				% [i, indicator.global_position, expected_positions[i]]
			)
		)
		...
		(
			...
			. append_failure_message("Indicator %d X position should match tile center" % i)
			. is_equal_approx(expected_positions[i].x, 0.1)
		)
		(
			assert_that(indicator.global_position.y)
			. append_failure_message("Indicator %d Y position should match tile center" % i)
			. is_equal_approx(expected_positions[i].y, 0.1)
		)
