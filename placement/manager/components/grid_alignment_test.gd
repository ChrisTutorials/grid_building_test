...
func test_off_grid_positions_get_aligned():
	...
	for test_case in test_cases:
		...
		var expected_aligned_position = tile_map_layer.to_global(
			tile_map_layer.map_to_local(input_tile)
		)
...
func test_tile_coordinate_calculation_consistency():
	...
	var world_positions = [Vector2(100, 200), Vector2(543.7, 678.3), Vector2(-50, -80)]
	...
	for world_pos in world_positions:
		...
		for i in range(1, tile_coords.size()):
			(
				assert_vector(Vector2(tile_coords[i]))
				. append_failure_message(
					(
