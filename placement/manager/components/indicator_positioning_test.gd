...
func before_test():
	...
	# Create positioner and set it to a specific target position using GodotTestFactory
...
func test_indicator_world_positioning():
	...
	for test_case in test_cases:
		...
		print(
			(
				"Tile pos: %s -> World pos: %s (expected %s)"
				% [tile_pos, actual_world_pos, expected_world_pos]
			)
		)

		# Check that the conversion produces the expected world position
		(
			assert_that(actual_world_pos.x)
			. append_failure_message(
				"World X position should match expected for tile %s" % tile_pos
			)
			. is_equal_approx(expected_world_pos.x, 0.1)
		)
		(
			assert_that(actual_world_pos.y)
			. append_failure_message(
				"World Y position should match expected for tile %s" % tile_pos
			)
