...
func _create_simple_trapezoid_parented() -> CollisionPolygon2D:
...
func test_trapezoid_coverage_matches_geometry() -> void:
    var poly = _create_simple_trapezoid_parented()
    var dict = mapper._get_tile_offsets_for_collision_polygon(poly, tile_map_layer)
