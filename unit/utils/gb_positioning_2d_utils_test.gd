...
func test_get_tile_from_global_position_basic() -> void:
	...
	assert_that(result_tile).is_equal(expected_tile).append_failure_message(
...
func test_get_tile_from_global_position_edge_cases() -> void:
	...
	assert_that(zero_tile).is_equal(Vector2i(0, 0)).append_failure_message(
		"Zero position should map to tile (0,0), got %s" % str(zero_tile)
	)
	assert_that(negative_tile).is_equal(Vector2i(-2, -1)).append_failure_message(
...
func test_move_to_tile_center_basic() -> void:
	...
	assert_that(result_tile).is_equal(target_tile).append_failure_message(
		"Function should return the target tile %s, got %s" % [str(target_tile), str(result_tile)]
	)
	# Verify node was positioned (not at zero) and the tile calculation works
	assert_that(test_node.global_position).is_not_equal(Vector2.ZERO).append_failure_message(
	...
	assert_that(result_tile).is_equal(target_tile).append_failure_message(
		"Function should return the input tile %s, got %s" % [str(target_tile), str(result_tile)]
	)
...
func test_get_tile_from_node_position_basic() -> void:
	...
	assert_that(result_tile).is_equal(expected_tile).append_failure_message(
		"Node at position %s should be on tile %s, got %s" % [str(test_node.global_position), str(expected_tile), str(result_tile)]
	)
...
func test_get_tile_from_node_position_null_safety() -> void:
	...
	assert_that(result_null_node).is_equal(Vector2i.ZERO).append_failure_message(
		"Null node should return Vector2i.ZERO, got %s" % str(result_null_node)
	)
	assert_that(result_null_map).is_equal(Vector2i.ZERO).append_failure_message(
		"Null map should return Vector2i.ZERO, got %s" % str(result_null_map)
	)
	assert_that(result_both_null).is_equal(Vector2i.ZERO).append_failure_message(
		"Both null should return Vector2i.ZERO, got %s" % str(result_both_null)
	)
...
func test_move_node_by_tiles_basic() -> void:
	...
	assert_that(result_tile).is_equal(expected_tile).append_failure_message(
		"Node moved by delta %s should end up on tile %s, got %s" % [str(tile_delta), str(expected_tile), str(result_tile)]
	)
...
func test_move_node_by_tiles_scenarios(
	...
	assert_that(result_tile).is_equal(expected_tile).append_failure_message(
		...
		[test_name, str(start_position), str(tile_delta), str(expected_tile), str(result_tile)]
	)
...
func test_snap_tile_to_region_basic() -> void:
	...
	assert_that(snapped_inside).is_equal(inside_tile).append_failure_message(
		"Tile inside region should remain unchanged: %s" % str(inside_tile)
	)
...
func test_snap_tile_to_region_invalid_region() -> void:
	...
	assert_that(result).is_equal(test_tile).append_failure_message(
		"Invalid region should return original tile unchanged: %s" % str(test_tile)
	)
...
func test_viewport_center_to_world_position_with_camera() -> void:
	...
	assert_that(world_position).is_not_equal(Vector2.ZERO).append_failure_message(
		"World position should be calculated, got %s" % str(world_position)
	)
...
func test_move_node_to_tile_at_viewport_center() -> void:
	...
	assert_object(result_tile).is_not_null().append_failure_message(
...
func test_direction_to_tile_delta_scenarios(
	...
	assert_that(result_delta).is_equal(expected_delta).append_failure_message(
		"Test %s: Direction %s should convert to delta %s, got %s" % 
		[test_name, str(input_direction), str(expected_delta), str(result_delta)]
	)
...
func test_coordinate_conversion_roundtrip() -> void:
	...
	assert_that(final_position).is_not_equal(Vector2.ZERO).append_failure_message(
	...
	assert_that(tile_coord.x).is_greater_equal(0)
	assert_that(tile_coord.y).is_greater_equal(0)
...
func test_positioning_utilities_dry_compliance() -> void:
	...
	assert_object(tile_coord).is_not_null().append_failure_message("get_tile_from_node_position should return Vector2i")
	assert_object(world_pos).is_not_null().append_failure_message("viewport_center_to_world_position should return Vector2") 
	assert_bool(region_valid).append_failure_message("is_region_valid should return bool")
