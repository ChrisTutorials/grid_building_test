...
func _create_mock_grid_template() -> PackedScene:
	var template: PackedScene = auto_free(PackedScene.new())
	var grid_container: GridContainer = auto_free(GridContainer.new())
	grid_container.name = "MockGridContainer"
	grid_container.columns = 1  # Default single column layout for testing
	template.pack(grid_container)
	return template
...
func _create_mock_placeable_entry_template() -> PackedScene:
	var template: PackedScene = auto_free(PackedScene.new())
	var entry_panel: PanelContainer = auto_free(PanelContainer.new())
	entry_panel.name = "MockPlaceableEntry"
	# Add a placeable property and signal to mimic PlaceableView behavior
	entry_panel.set_script(load("res://addons/grid_building/ui/placeable/single/placeable_view.gd"))
	template.pack(entry_panel)
	return template
...
func _create_mock_sequence_entry_template() -> PackedScene:
	var template: PackedScene = auto_free(PackedScene.new())
	var entry_panel: PanelContainer = auto_free(PanelContainer.new())
	entry_panel.name = "MockSequenceEntry"
	# Add sequence property and signals to mimic PlaceableListEntry behavior
	entry_panel.set_script(load("res://addons/grid_building/ui/placeable/sequence/placeable_list_entry.gd"))
	template.pack(entry_panel)
	return template
