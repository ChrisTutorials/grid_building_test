...
func test_sequence_creation_and_properties() -> void:
	...
	assert_str(sequence.display_name).is_equal("Tower Variants")
	assert_int(sequence.placeables.size()).is_equal(3)
	assert_object(sequence.placeables[0]).is_same(test_placeable_1)
	assert_object(sequence.placeables[1]).is_same(test_placeable_2)
	assert_object(sequence.placeables[2]).is_same(test_placeable_3)
