...
func test_collision_geometry_utils_transform_building() -> void:
	...
	var static_body = StaticBody2D.new()
	static_body.position = Vector2position
	test_obj.add_child(static_body)
...
func test_string_utilities_separator_matching() -> void:
	...
	assert_that(GBString.match_num_seperator("_", 2)).is_true()  # Underscore
	assert_that(GBString.match_num_seperator("-", 3)).is_true()  # Dash
	assert_that(GBString.match_num_seperator(" ", 1)).is_true()  # Space
	assert_that(GBString.match_num_seperator("_", 0)).is_false()  # None
	assert_that(GBString.match_num_seperator("_", 1)).is_false()  # Space
	assert_that(GBString.match_num_seperator("_", 3)).is_false()  # Dash
	assert_that(GBString.match_num_seperator("-", 0)).is_false()  # None
	assert_that(GBString.match_num_seperator("-", 1)).is_false()  # Space
	assert_that(GBString.match_num_seperator("-", 2)).is_false()  # Underscore
	assert_that(GBString.match_num_seperator(" ", 0)).is_false()  # None
	assert_that(GBString.match_num_seperator(" ", 2)).is_false()  # Underscore
	assert_that(GBString.match_num_seperator(" ", 3)).is_false()  # Dash
	
	# Test new get_separator_string function
	assert_that(GBString.get_separator_string(0)).is_equal("")
	assert_that(GBString.get_separator_string(1)).is_equal(" ")
	assert_that(GBString.get_separator_string(2)).is_equal("_")
	assert_that(GBString.get_separator_string(3)).is_equal("-")
...
func test_performance_utilities_combined() -> void:
	...
	assert_that(elapsed).append_failure_message("Combined utilities performance test completed in " + str(elapsed) + " microseconds").is_less(100000)  # Should complete in under 0.1 seconds
