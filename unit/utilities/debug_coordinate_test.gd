...
func test_debug_coordinate_transformation() -> void:
	...
	assert_that(polygon.position).is_equal(POLYGON_POSITION).override_failure_message("Polygon position should be set correctly")
	assert_that(polygon.global_position).is_equal(POLYGON_POSITION).override_failure_message("Polygon global_position should match position when no parent transform")
	assert_that(polygon.polygon).is_equal(polygon_points).override_failure_message("Polygon points should match input")
	assert_that(test_map.tile_set.tile_size).is_equal(TILE_SIZE).override_failure_message("Tilemap should have 16x16 tile size")
	...
	assert_that(center_tile).is_equal(CENTER_TILE).override_failure_message("Center tile should be (20,20) for position (320,320) with 16px tiles")
	...
	assert_that(tile_size).is_equal(Vector2(TILE_SIZE)).override_failure_message("Tile size should be 16x16")
