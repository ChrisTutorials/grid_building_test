...
func test_zoom_level_conversion_scenarios(
	...
	# Test conversion of offset screen position
	var screen_offset: Vector2 = Vector2(50, 50)
	var world_pos: Vector2 = GBPositioning2DUtils.convert_screen_to_world_position(screen_offset, viewport)
	
	# Assert: World position should scale with zoom level
	var expected_offset: Vector2 = screen_offset * expected_scale
	assert_vector(world_pos).is_equal_approx(expected_offset, Vector2(2.0, 2.0)).append_failure_message(
		"Screen offset %s at zoom %s should convert to world offset %s, got %s" % [str(screen_offset), str(zoom_level), str(expected_offset), str(world_pos)]
	)
...
func test_tile_center_positioning_accuracy() -> void:
	...
	var tile_center_with_offset: Vector2 = tile_center_local + tile_size * 0.5
	var tile_center_global: Vector2 = tile_map.to_global(tile_center_with_offset)
	...
	assert_float(distance_to_center).is_less_equal(2.0).append_failure_message(
		"Screen pos %s -> world pos %s should be close to tile center %s (tile %s), distance: %s" % [str(screen_pos), str(world_pos), str(tile_center_global), str(tile_coord), str(distance_to_center)]
	)
...
func test_multiple_screen_positions_tile_alignment() -> void:
	...
	for i in range(test_positions.size()):
		...
		assert_bool(tile_coord.x >= -10 and tile_coord.x <= 20).is_true().append_failure_message(
			"Position %d: screen %s -> tile x %d should be in reasonable range (-10 to 20)" % [i, str(screen_pos), tile_coord.x]
		)
		
		assert_bool(tile_coord.y >= -10 and tile_coord.y <= 20).is_true().append_failure_message(
			"Position %d: screen %s -> tile y %d should be in reasonable range (-10 to 20)" % [i, str(screen_pos), tile_coord.y]
		)
...
## Test: coordinate conversion handles missing camera gracefully
## Setup: Viewport without Camera2D
## Act: Attempt coordinate conversion
## Assert: Fallback behavior works without crashing
func test_missing_camera_fallback() -> void:
	# Setup: Create viewport without camera
	var test_viewport: SubViewport = SubViewport.new()
	add_child(test_viewport)
	auto_free(test_viewport)
	
	# Test conversion without camera (should not crash)
	var screen_pos: Vector2 = Vector2(100, 100)
	var world_pos: Vector2 = GBPositioning2DUtils.convert_screen_to_world_position(screen_pos, test_viewport)
	
	# Assert: Should return some valid position (fallback behavior)
	assert_vector(world_pos).is_not_null().append_failure_message(
		"Coordinate conversion without camera should return valid fallback position, got null"
	)
	
	# Assert: Should return reasonable fallback (likely the screen position itself)
	var distance_from_input: float = world_pos.distance_to(screen_pos)
	assert_float(distance_from_input).is_less_equal(100.0).append_failure_message(
		"Fallback world position %s should be reasonably close to input screen position %s, distance: %s" % [str(world_pos), str(screen_pos), str(distance_from_input)]
	)
