...
func test_geometry_debug_scenarios_with_tile_shapes(
	...
	var intersection_area: float = GBGeometryMath.intersection_area_with_tile(polygon_data, tile_pos, tile_size, tile_shape)
	var has_overlap: bool = intersection_area > OVERLAP_THRESHOLD
	
	# Validate expected behavior
	assert_bool(has_overlap).append_failure_message(
		"Geometry %s with %s tiles: expected overlap %s but got %s (area: %.4f)" % [polygon_name, _tile_shape_name(tile_shape), expected_overlap, has_overlap, intersection_area]
...
func test_geometry_debug_scenarios(
	...
	var polygon_bounds: Rect2 = GBGeometryMath.get_polygon_bounds(polygon_data)
	var intersection_area: float = GBGeometryMath.intersection_area_with_tile(polygon_data, tile_pos, tile_size, TileSet.TILE_SHAPE_SQUARE)
	var has_overlap: bool = intersection_area > OVERLAP_THRESHOLD
...
func test_concave_polygon_void_handling() -> void:
	...
	var poly: PackedVector2Array = PackedVector2Array([Vector2(0,0), Vector2(12,0), Vector2(12,4), Vector2(4,4), 
...
func test_correct_trapezoid_geometry() -> void:
	...
	var local_trapezoid_points: PackedVector2Array = PackedVector2Array([Vector2(-16, 8), Vector2(-8, -8), Vector2(8, -8), Vector2(16, 8)])
...
func test_does_polygon_overlap_tile_for_all_shapes(
	...
	var overlaps: bool = GBGeometryMath.does_polygon_overlap_tile(polygon, tile_pos, tile_size, tile_shape, OVERLAP_THRESHOLD)
...
func test_gb_geometry_math_edge_cases() -> void:
	...
	# Empty polygon
	var empty_polygon: PackedVector2Array = PackedVector2Array([])
	var empty_area: float = GBGeometryMath.intersection_area_with_tile(empty_polygon, tile_pos, tile_size, TileSet.TILE_SHAPE_SQUARE)
	assert_float(empty_area).is_equal(0.0)
	
	# Single point
	var point_polygon: PackedVector2Array = PackedVector2Array([Vector2(8, 8)])
	var point_area: float = GBGeometryMath.intersection_area_with_tile(point_polygon, tile_pos, tile_size, TileSet.TILE_SHAPE_SQUARE) 
