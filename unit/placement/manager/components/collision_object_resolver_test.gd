...
func collision_resolution_test_data() -> Array:
	return [
		...
		["collision_shape_2d_with_parent", func() -> CollisionResolutionTestData: return _setup_collision_shape_with_parent(), true, ""],
		["collision_polygon_2d_with_parent", func() -> CollisionResolutionTestData: return _setup_collision_polygon_with_parent(), true, ""],
		["collision_shape_2d_without_parent", func() -> CollisionResolutionTestData: return _setup_collision_shape_without_parent(), false, "has no CollisionObject2D parent"],
		["collision_polygon_2d_without_parent", func() -> CollisionResolutionTestData: return _setup_collision_polygon_without_parent(), false, "has no CollisionObject2D parent"],
		["unsupported_node_type", func() -> CollisionResolutionTestData: return _setup_unsupported_node(), false, "Unsupported collision node type"],
		["null_node", func() -> CollisionResolutionTestData: return _setup_null_node(), false, "Collision node is null"],
		["collision_object_2d_without_test_setup", func() -> CollisionResolutionTestData: return _setup_collision_object_without_test_setup(), false, "No test setup found"]
	]

## Parameterized test for collision object resolution
@warning_ignore("unused_parameter")
func test_collision_object_resolution(test_name: String, setup_func: Callable, expected_valid: bool, expected_error_contains: String, test_parameters := collision_resolution_test_data()) -> void:
	...
	# Assert
	assert_that(result.is_valid).is_equal(expected_valid)
	if expected_valid:
		assert_that(result.collision_object).is_same(expected_collision_obj)
		assert_that(result.test_setup).is_same(expected_test_setup)
		assert_that(result.error_message).is_empty()
	else:
		assert_that(result.collision_object).is_same(expected_collision_obj)
		assert_that(result.test_setup).is_same(expected_test_setup)
		assert_that(result.error_message).contains(expected_error_contains)
...
func test_object_matches_layer_mask() -> void:
	...
	# Test matching masks
	assert_that(_resolver.object_matches_layer_mask(collision_obj, 1)).is_true()   # 001 & 101 = 001
	assert_that(_resolver.object_matches_layer_mask(collision_obj, 4)).is_true()   # 100 & 101 = 100
	assert_that(_resolver.object_matches_layer_mask(collision_obj, 2)).is_false()  # 010 & 101 = 000
	
	# Test null object
	assert_that(_resolver.object_matches_layer_mask(null, 1)).is_false()
