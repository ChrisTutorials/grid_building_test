...
func before_test() -> void:
	...
	assert_that(env).is_not_null().append_failure_message(
		"Building test environment must instantiate successfully"
	)
...
func _enter_build_mode_for_rect_4x2_and_start_drag() -> Dictionary:
	...
	var report: PlacementReport = _building_system.enter_build_mode(
...
func _assert_build_attempted(context: String = "") -> void:
	var msg := (
		"Expected at least one build attempt %s. success=%d failed=%d"
...
func _expect_placements(expected: int, context: String = "") -> void:
	...
	var issues_str: String = (
		str(_last_build_report.get_issues()) if _last_build_report != null else "[]"
	)
	var msg: String = (
		...
		% [
			expected,
			ctx,
			_placed_positions.size(),
			_build_success_count,
			_build_failed_count,
			issues_str,
			str(_placed_positions)
		]
	)
	assert_int(_placed_positions.size()).append_failure_message(_format_debug(msg)).is_equal(
		expected
	)
...
func test_placement_validation_with_rules(
	...
	(
		...
		. append_failure_message(
			(
				...
				+ "env.placer="
				+ (str(env.placer) if env else "env is null")
				+ ", "
				+ "user_node="
				+ str(user_node)
				+ ", "
	...
	var diagnostic_details := _format_validation_failure_details(
		rule_scenario, rule_type, expected_valid, result, test_rules
	)
	assert_that(result.is_successful()).append_failure_message(diagnostic_details).is_equal(
		expected_valid
	)
...
func _setup_blocking_collision() -> void:
	...
	logger.log_verbose(
		"Created blocking collision body at position: %s" % blocking_body.global_position
	)
...
func _debug_collision_detection() -> void:
	...
	for i in range(indicators.size()):
		...
		logger.log_verbose(
			"Indicator[" + str(i) + "] collision_mask: " + str(indicator.collision_mask)
		)
		logger.log_verbose(
			"Indicator[" + str(i) + "] is_colliding: " + str(indicator.is_colliding())
		)
		logger.log_verbose(
			"Indicator[" + str(i) + "] get_collision_count: " + str(indicator.get_collision_count())
		)

		# Check if blocking body would be detected
		if blocking_bodies.size() > 0:
			var blocking_body: StaticBody2D = blocking_bodies[0] as StaticBody2D
			var collision_matches: bool = (
				(blocking_body.collision_layer & indicator.collision_mask) != 0
			)
			logger.log_verbose(
				(
					"Indicator["
					+ str(i)
					+ "] collision_mask & blocking_layer match: "
					+ str(collision_matches)
				)
			)
			...
			logger.log_verbose(
				(
					...
					+ str(i)
					+ "] exceptions count: "
					+ str(indicator.get_exception_count())
				)
			)
			...
			logger.log_verbose(
				(
					"Indicator["
					+ str(i)
					+ "] after force_update is_colliding: "
					+ str(indicator.is_colliding())
				)
			)


func _collect_offsets(
	mapper: CollisionMapper, poly: CollisionPolygon2D, tile_map: TileMapLayer
) -> Array[Vector2i]:
	var node_tile_offsets: Dictionary = mapper.get_tile_offsets_for_collision_polygon(
		poly, tile_map
	)
	(
	...
	for k: Vector2i in node_tile_offsets.keys():
		arr.append(k)
	arr.sort()
	...
	if arr.is_empty():
		...
		if typeof(PolygonTileMapper) != TYPE_NIL:
			...
			diag_msg = (
				"; diag.initial="
				+ str(diag.initial_offset_count)
				+ ", diag.final="
				+ str(diag.final_offset_count)
				+ ", diag.was_parented="
				+ str(diag.was_parented)
				+ ", diag.was_convex="
				+ str(diag.was_convex)
			)

			# Add coordinate diagnostics
			var diag_center_tile: Vector2i = tile_map.local_to_map(
				tile_map.to_local(poly.global_position)
			)
			var polygon_world_center: Vector2 = poly.global_position
			var polygon_tile: Vector2i = tile_map.local_to_map(
				tile_map.to_local(polygon_world_center)
			)
			...
			diag_msg += (
				"; center_tile="
				+ str(diag_center_tile)
				+ ", poly_world="
				+ str(polygon_world_center)
				+ ", poly_tile="
				+ str(polygon_tile)
				+ ", tile_size="
				+ str(diag_tile_size)
		...
		(
			...
			. append_failure_message(
				(
					...
					+ str(node_tile_offsets.keys())
					+ ", Dict size: "
					+ str(node_tile_offsets.size())
...
func _collect_placement_diagnostics(context: String = "") -> String:
	...
	var target_pos: String = (
		"null"
		if _targeting_state.get_target() == null
		else str(_targeting_state.get_target().global_position)
	)
...
func _format_validation_failure_details(
	scenario: String,
	rule_type: String,
	expected: bool,
	...
	details.append(
		"  • Expected result: " + str(expected) + ", Got: " + str(result.is_successful())
	)
	...
	details.append(
		(
			...
			+ str(_is_position_within_map_bounds(_positioner.global_position))
		)
	)
...
func _analyze_rectangular_building_coverage_problem(
	...
	rect_height: float,
	expected_total: int,
	actual_tiles: Array[Vector2i],
	expected_tiles: Array[Vector2i],
	missing_tiles: Array[Vector2i],
	extras_diag: String
) -> String:
	...
	analysis.append(
		(
			"  • EXPECTED TILES: "
			+ str(expected_tiles.size())
			+ " tiles in pattern "
			+ str(expected_tiles)
		)
	)
	analysis.append(
		"  • ACTUAL TILES: " + str(actual_tiles.size()) + " tiles in pattern " + str(actual_tiles)
	)
	analysis.append(
		"  • MISSING TILES: " + str(missing_tiles.size()) + " tiles: " + str(missing_tiles)
	)
	...
	if actual_tiles.size() < expected_total / 2.0:
		...
		analysis.append(
			"  • Partial collision detection - shape may be incorrectly sized or positioned"
		)
	...
	if test_objects.size() > 0:
		for i in range(test_objects.size()):
			...
			if obj is StaticBody2D:
				...
				analysis.append(
					"  • Test body[" + str(i) + "] world pos: " + str(body.global_position)
				)
				analysis.append(
					"  • Test body[" + str(i) + "] collision layer: " + str(body.collision_layer)
				)
				...
				for j in range(shapes.size()):
					...
					if shape_node is CollisionShape2D:
						...
						if coll_shape.shape is RectangleShape2D:
							...
							analysis.append(
								"  • Collision shape[" + str(j) + "] size: " + str(rect.size)
							)
							analysis.append(
								(
									"  • Shape world bounds: "
									+ str(body.global_position - rect.size / 2)
									+ " to "
									+ str(body.global_position + rect.size / 2)
								)
							)
...
func _analyze_tile_count_problem(
	...
	analysis.append(
		"  • SHORTFALL: " + str(expected_count - actual_tiles.size()) + " tiles missing"
	)
...
func test_large_rectangle_generates_full_grid_of_indicators() -> void:
	...
	logger.log_info(
		(
			"  • Shape world bounds: "
			+ str(test_building.global_position - rect_shape.size / 2)
			+ " to "
			+ str(test_building.global_position + rect_shape.size / 2)
		)
	)
	...
	for t in tiles:
		var inside := (
			t.x >= exp_min_x and t.x <= exp_max_x and t.y >= exp_min_y and t.y <= exp_max_y
		)
		if not inside:
			if t.y < exp_min_y:
				extras_top.append(t)
			elif t.y > exp_max_y:
				extras_bottom.append(t)
			elif t.x < exp_min_x:
				extras_left.append(t)
			elif t.x > exp_max_x:
				extras_right.append(t)
	...
	var center_bottom_analysis := _analyze_center_bottom_tile_problem(
		bottom_middle, tiles, missing, problem_analysis
	)
	...
	assert_int(tiles.size()).append_failure_message(count_analysis).is_greater_equal(
		RECT_EXPECTED_TILES
	)
...
func test_isolated_rect_48x64_tile_offsets() -> void:
	...
	if typeof(PolygonTileMapper) != TYPE_NIL:
		...
		mapper_diag = (
			"; diag.initial=%s, diag.final=%s, did_expand_trapezoid=%s, was_convex=%s"
			% [
				str(d.initial_offset_count),
				str(d.final_offset_count),
				str(d.did_expand_trapezoid),
				str(d.was_convex)
			]
		)

	var failure_msg: String = (
		"Isolated %sx%s polygon -> tile offsets mismatch:\n"
	...
	failure_msg += (
		"  • Map used_rect: %s; tile_size: %s; positioner: %s"
		% [_map.get_used_rect(), tile_size, str(_positioner.global_position)]
	)
	failure_msg += mapper_diag

	# Assert exact tile set equality (order-independent) with detailed diagnostics on failure
	assert_array(offsets).append_failure_message(failure_msg).contains_exactly_in_any_order(
		expected_tiles
	)
...
func _disabled_test_building_mode_enter_exit() -> void:
	# Enter build mode
	var enter_report: PlacementReport = _building_system.enter_build_mode(
...
func test_building_placement_attempt() -> void:
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Enter build mode should succeed with preview collision shapes only (no dual collision issue): issues=%s, config=%s"
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Build attempt should succeed without collision shape interference - Position: %s, Target collision children: %d, Issues: %s"
					% [
						str(_positioner.global_position),
						user_node.get_child_count(),
						str(placement_result.get_issues())
					]
				)
			)
		) \
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Build attempt should return valid placed object - Success count: %d, Failed count: %d"
...
func test_building_state_transitions() -> void:
	...
	assert_bool(initial_state).append_failure_message("Should not start in build mode").is_false()
...
func test_single_placement_per_tile_constraint() -> void:
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Enter build mode should succeed without collision shape conflicts: issues=%s, target_children=%d"
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"First placement should succeed without dual collision interference - Position: %s, Issues: %s"
...
func test_complete_building_workflow() -> void:
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Enter build mode should succeed in complete workflow (no collision shape setup on target): issues=%s, config=%s"
					% [str(enter_report.get_issues()), _get_container_config_debug()]
				)
			)
		) \
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Build should succeed without collision conflicts - Position: %s, Target children: %d, Preview collision managed separately, Issues: %s"
					% [
						str(_positioner.global_position),
						user_node.get_child_count(),
						str(build_report.get_issues())
					]
				)
			)
		) \
	...
	(
		...
		. append_failure_message(
			_format_debug(
				(
					"Build report should contain valid placed object - Success: %d, Failed: %d, No dual collision setup"
					% [_build_success_count, _build_failed_count]
				)
			)
		) \
...
func _prepare_target_for_successful_build(_tile: Vector2i = SAFE_LEFT_TILE) -> void:
	...
	(
		...
		. log_debug(
			(
				"Positioned at tile %s (world: %s) within map bounds %s (no collision shapes added to target)"
...
func _setup_test_object_collision_shapes() -> void:
	...
	var existing_body: StaticBody2D = (
		user_node.get_node_or_null("TestCollisionBody") as StaticBody2D
	)
...
func _setup_target_collision_shape_for_validation() -> void:
	...
	var existing_body: StaticBody2D = (
		_targeting_state.get_target().get_node_or_null("ValidationCollisionBody") as StaticBody2D
	)
	if existing_body != null:
	...
	logger.log_verbose(
		(
			"Added validation collision body with shape to target: %s"
			% _targeting_state.get_target().name
		)
	)
...
func _find_valid_tile_within_map_bounds() -> Vector2i:
	...
	logger.log_warning(
		(
			"No valid tiles found in map bounds %s, using center tile %s"
			% [str(used_rect), str(center_tile)]
		)
	)
	return center_tile
