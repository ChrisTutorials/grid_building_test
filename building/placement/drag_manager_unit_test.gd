...
func test_start_drag_creates_drag_data() -> void:
	...
	(
		...
		. append_failure_message("start_drag() should return DragPathData") \
		. is_not_null()
	)
	(
		assert_bool(_drag_manager.is_dragging()) \
		. append_failure_message("is_dragging() should return true") \
		. is_true()
	)
...
func test_stop_drag_clears_drag_data() -> void:
	...
	(
		...
		. append_failure_message("drag_data should be null") \
		. is_null()
	)
	(
		assert_bool(_drag_manager.is_dragging()) \
		. append_failure_message("is_dragging() should return false") \
		. is_false()
	)
...
func test_cannot_start_drag_while_already_dragging() -> void:
	...
	(
		...
		. append_failure_message("start_drag() should return null when already dragging") \
		. is_null()
	)
	(
		assert_object(_drag_manager.drag_data) \
		. append_failure_message("drag_data should still be the first drag") \
		. is_same(first_drag)
	)
...
func test_set_test_mode_controls_input_processing() -> void:
	(
		...
		. append_failure_message("Input processing should be enabled by default") \
		. is_true()
	)
	_drag_manager.set_test_mode(true)
	(
		assert_bool(_drag_manager.is_processing_input()) \
		. append_failure_message("Test mode should disable input processing") \
		. is_false()
	)
	_drag_manager.set_test_mode(false)
	(
		assert_bool(_drag_manager.is_processing_input()) \
		. append_failure_message("Disabling test mode should re-enable input") \
		. is_true()
	)
...
func test_format_drag_state_helper() -> void:
	...
	(
		...
		. append_failure_message("format_drag_state should return non-empty string") \
		. is_not_empty()
	)
	(
		assert_bool(formatted.contains("DragState")) \
		. append_failure_message("Should contain 'DragState'") \
		. is_true()
	)
