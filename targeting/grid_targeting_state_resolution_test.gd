...
func test_setting_target_emits_both_signals() -> void:
	...
	var signal_data := {"target_changed": false, "target_new": null}

	# Connect to target_changed signal
	_targeting_state.target_changed.connect(
		func(new_target: Node2D, _old: Node2D) -> void:
...
func test_setting_same_target_twice_doesnt_emit_signals() -> void:
	...
	_targeting_state.target_changed.connect(
		func(_new: Node2D, _old: Node2D) -> void: signal_data[0] += 1
	)
	...
	assert_int(first_count).append_failure_message("First target set should emit signal").is_equal(
		1
	)
