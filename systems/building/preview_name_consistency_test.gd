...
func test_same_placeable_twice_preserves_name():
	...
	var preview1 = system._states.building.preview
	...
	var preview2 = system._states.building.preview
	assert_object(preview2).is_not_null()
	assert_str(preview2.name).override_failure_message(
		"Preview name should remain '%s' but got '%s' on second assignment" % [expected_name, preview2.name]
...
func test_different_placeables_have_correct_names():
	...
	system.enter_build_mode(placeable1)
	var preview3 = system._states.building.preview
