...
var manipulatable_settings_all_allowed: ManipulatableSettings = (
	GBTestConstants.MANIPULATABLE_SETTINGS_ALL_ALLOWED
)
var manipulatable_settings_none_allowed: ManipulatableSettings = (
	GBTestConstants.MANIPULATABLE_SETTINGS_NONE_ALLOWED
)
...
func test_start_move(
	...
	p_expected: bool,
	test_parameters := [
		[null, true], [rules_2_rules_1_tile_check, true], [manipulatable_settings_all_allowed, true]
	]
) -> void:
	...
	(
		...
		. append_failure_message("Source manipulatable should not be null")
		. is_not_null()
	)
	(
		assert_that(source_manipulatable.root)
		. append_failure_message("Source manipulatable root should not be null")
		. is_not_null()
	)
	...
	if result_data != null:
		...
		(
			assert_bool(result)
			. append_failure_message(
				"Move operation result should match expected value %s" % p_expected
			)
			. is_equal(p_expected)
		)
		...
		if (
			result_data.source != null
			and result_data.move_copy != null
			and _container.get_states().targeting.positioner != null
		):
			var positioner_pos: Vector2 = (
				_container.get_states().targeting.positioner.global_position
			)
			(
				assert_vector(result_data.move_copy.root.global_position)
				. append_failure_message(
					"Move copy position should match positioner position during move"
				)
				. is_equal(positioner_pos)
			)
...
func test_cancel() -> void:
	...
	(
		assert_that(move_result)
		. append_failure_message("try_move for cancel test should return valid result")
		. is_not_null()
	)
	...
	if move_result != null:
		...
		(
			...
			. append_failure_message("Move should be successfully started for cancel test")
			. is_true()
		)
		...
		if active_data != null and active_data.move_copy != null:
			...
			(
				...
				. append_failure_message(
					"Origin x position should be approximately %f" % TEST_POSITION.x
				)
			...
			(
				...
				. append_failure_message(
					"Origin y position should be approximately %f" % TEST_POSITION.y
				)
			...
			(
				...
				. append_failure_message("Manipulation data should be null after cancel")
				. is_null()
			)
			(
				assert_vector(active_data.source.root.global_position)
				. append_failure_message("Source position should return to original after cancel")
...
func test_try_move(
	...
	(
		assert_that(result_data)
		. append_failure_message("try_move should never return null, even for invalid inputs")
		. is_not_null()
	)

	if result_data != null:
		(
			assert_int(result_data.status)
			. append_failure_message(
				(
					"try_move status should be %s for target '%s', but got %s"
					% [p_expected, p_move_target, result_data.status]
				)
			)
			. is_equal(p_expected)
		)
...
func test_demolish(
	...
	test_parameters := [
		[manipulatable_settings_none_allowed, false], [manipulatable_settings_all_allowed, true]
	]
) -> void:
	# Use default value when p_settings is null
	if p_settings == null:
		p_settings = manipulatable_settings_all_allowed
		p_expected = true
	var target_manipulatable: Manipulatable = (
		_create_test_manipulatable(p_settings) if p_settings != null else all_manipulatable
	)
	(
		assert_that(target_manipulatable)
		. append_failure_message("Target manipulatable should not be null for demolish test")
		. is_not_null()
	)
	...
	(
		assert_bool(success)
		. append_failure_message(
			"Demolish operation should have expected success status %s" % p_expected
		)
		. is_equal(p_expected)
	)
...
func test_try_placement(
	...
	(
		assert_that(source)
		. append_failure_message("Source manipulatable should not be null for placement test")
		. is_not_null()
	)
	...
	(
		assert_that(move_result)
		. append_failure_message("try_move for placement test should return valid result")
		. is_not_null()
	)
	...
	if move_result != null:
		...
		(
			assert_bool(started)
			. append_failure_message("Move should be successfully started for placement test")
			. is_true()
		)
		...
		if move_data != null and move_data.move_copy != null:
			...
			(
				...
				. append_failure_message("Placement results should not be null")
				. is_not_null()
			)
			...
			if placement_results != null:
				...
				if success_status:
					(
						...
						. append_failure_message(
							"Move copy should be null after successful placement"
						)
						. is_null()
					)
					(
						...
						. append_failure_message(
							"Source position should match test location after placement"
						)
...
func test_failed_placement_with_invalid_move_data_cleans_up() -> void:
	...
	(
		assert_vector(original_source_position)
		. append_failure_message("Source position should match test position initially")
		. is_equal(TEST_POSITION)
	)
	...
	(
		...
		. append_failure_message("Move data status should be FAILED after invalid move data")
	...
	(
		...
		. append_failure_message(
			(
				...
				% [str(original_source_position), str(source.root.global_position)]
			)
		)
	...
	if is_instance_valid(move_copy_root):
		(
			assert_bool(move_copy_root.is_inside_tree())
			. append_failure_message(
	...
	if current_data != null:
		...
		(
			assert_int(current_data.status)
			. append_failure_message("Manipulation data should be FAILED status")
			. is_equal(GBEnums.Status.FAILED)
		)
...
func test_flip_vertical(
	p_manipulatable: Manipulatable, test_parameters := [[all_manipulatable]]  # Disabled for GdUnit4 parsing
) -> void:
...
func test_rotate_node2d_target_rotates_correctly(
	...
	for i in range(ROTATION_ITERATIONS):
		...
		(
			assert_bool(success)
			. append_failure_message("Rotate operation should succeed on iteration %d" % i)
			. is_true()
		)
		...
		var actual_rotation: float = _normalize_rotation(
			manipulation_parent.global_rotation_degrees
		)
		...
		(
			...
			. append_failure_message(
				(
					"Rotation should match expected degrees on iteration %d (expected: %f, actual: %f)"
					% [i, normalized_expected, actual_rotation]
				)
			)
...
func test_rotate_negative(
	p_manipulatable: Manipulatable, p_expected: bool, test_parameters := [[all_manipulatable, true]]  # Disabled for GdUnit4 parsing
) -> void:
	...
	for i in range(ROTATION_ITERATIONS):
		...
		(
			assert_bool(system.rotate(target, rotation_per_time))
			. append_failure_message(
...
func _create_test_move_data(p_settings: ManipulatableSettings) -> ManipulationData:
	(
		...
		. append_failure_message("Container should not be null when creating test move data")
		. is_not_null()
	)
	...
	(
		...
		. append_failure_message("Source object root should not be null")
		. is_not_null()
	)
	...
	(
		...
		. append_failure_message("Target duplicate should not be null")
		. is_not_null()
	)
	...
	(
		...
		. append_failure_message("Manipulator node should not be null")
		. is_not_null()
	)

	var data: ManipulationData = ManipulationData.new(
		manipulator_node, source_obj, target_duplicate, GBEnums.Action.MOVE
	)
	(
		assert_that(data)
		. append_failure_message("Manipulation data should not be null after creation")
		. is_not_null()
	)
...
func _validate_manipulation_data(data: ManipulationData, context: String) -> void:
	assert_that(data).append_failure_message("%s should not be null" % context).is_not_null()

	if data != null:
		(
			assert_that(data.source)
			. append_failure_message("%s source should not be null" % context)
			. is_not_null()
		)

		(
			assert_that(data.move_copy)
			. append_failure_message("%s move_copy should not be null" % context)
			. is_not_null()
		)
		...
		if data.source != null and data.move_copy != null:
			(
				assert_that(data.source)
				. append_failure_message(
					"Source should be different instance from move_copy in %s" % context
				)
				. is_not_same(data.move_copy)
			)
			(
				assert_that(data.source.root)
				. append_failure_message(
					"Source root should be different instance from move_copy root in %s" % context
				)
				. is_not_same(data.move_copy.root)
			)


## Validates manipulatable for transform operations - prevents null errors
func _validate_manipulatable_for_transform(
	p_manipulatable: Manipulatable, operation: String
) -> void:
	(
		assert_that(p_manipulatable)
		. append_failure_message("Manipulatable for %s should not be null" % operation)
		. is_not_null()
	)

	if p_manipulatable != null:
		(
			assert_that(p_manipulatable.root)
			. append_failure_message("Manipulatable.root for %s should not be null" % operation)
			. is_not_null()
		)
