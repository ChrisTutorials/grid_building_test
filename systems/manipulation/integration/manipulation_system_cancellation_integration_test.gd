...
var manipulatable_settings: ManipulatableSettings = (
...
func test_cancel_without_active_manipulation_is_safe() -> void:
	...
	assert_object(manipulation_state.data).append_failure_message("Should remain null").is_null()
...
func test_multiple_cancellations_are_safe() -> void:
	...
	# Verify cancelled
	assert_object(manipulation_state.data).append_failure_message("Should be cancelled").is_null()
