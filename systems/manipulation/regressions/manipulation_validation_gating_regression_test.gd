...
@warning_ignore("unused_parameter") const ManipulationHelpers := preload("uid://ba4o2x6ctbwr")
...
func _create_test_manipulatable_with_collision_rules() -> Manipulatable:
	...
	var manipulatable: Manipulatable = ManipulationHelpers.create_test_manipulatable(
		self, "TestManipulatable", Vector2(32, 32), Vector2(32, 32), true  # with_move_rules = true
	)
...
func test_validation_failure_prevents_object_movement() -> void:
	...
	var manipulatable: Manipulatable = ManipulationHelpers.create_test_manipulatable(
		env, "TestManipulatable", Vector2(32, 32), Vector2(32, 32), true  # with_move_rules = true
	)
	...
	collision_body = ManipulationHelpers.create_collision_obstacle(
		env, target_position, Vector2(32, 32), 1  # Same collision layer as manipulatable body
	)
	...
	(
		...
		. append_failure_message("Obstacle should be in scene tree")
		. is_true()
	)
	(
		assert_int(collision_body.get_child_count())
		. append_failure_message("Obstacle should have CollisionShape2D child")
		. is_greater(0)
	)
	...
	if collision_body.get_child_count() > 0:
		...
		if shape_node:
			(
				...
				. append_failure_message("CollisionShape2D should have a shape")
				. is_not_null()
			)

	# Act: Start move operation
	var move_data: ManipulationData = manipulation_system.try_move(manipulatable.root)
	assert_that(move_data).append_failure_message("Move should start successfully").is_not_null()

	(
		assert_int(move_data.status)
		. append_failure_message(
			(
				"Move should be in STARTED status, got: %s"
				% ManipulationHelpers.format_status(move_data.status)
			)
		)
	...
	(
		assert_int(updated_count)
		. append_failure_message("Expected at least one indicator to be updated")
		. is_greater(0)
	)
	...
	(
		assert_that(validation_results)
		. append_failure_message("Validation results should not be null")
		. is_not_null()
	)

	(
		assert_bool(validation_results.is_successful())
		. append_failure_message(
			(
	...
	(
		...
		. append_failure_message(
			(
				"Move status should remain STARTED after validation failure to allow retry, got: %s"
				% ManipulationHelpers.format_status(move_data.status)
			)
		)
		. is_equal(GBEnums.Status.STARTED)
	...
	(
		assert_vector(manipulatable.root.global_position)
		. append_failure_message(
			(
...
func test_validation_success_allows_object_movement() -> void:
	...
	var manipulatable: Manipulatable = ManipulationHelpers.create_test_manipulatable(
		env, "TestManipulatable", Vector2(32, 32), Vector2(32, 32), false  # with_move_rules = false - no validation rules
	)
