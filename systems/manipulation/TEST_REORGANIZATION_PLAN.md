...
**Goal**: Restructure test hierarchy to leverage dependency injection pattern for faster, clearer tests.
...
func test_start_move_requires_valid_data() -> void:
    var invalid_data = ManipulationData.new(null, null, null, GBEnums.Action.MOVE)
...
func before_test() -> void:
    runner = scene_runner(GBTestConstants.MANIPULATION_TEST_ENV.resource_path)
...
## ROOT CAUSE: ManipulationParent.reset() cleared transforms before application
...
- [ ] Move integration tests from `integration/system_interactions/` → `manipulation/integration/`
- [ ] Move regression tests from `regressions/manipulation/` → `manipulation/regressions/`
- [ ] Move `placement/manipulation_move_positioner_regression_test.gd` → `manipulation/regressions/`
...
- `manipulation_cancellation_unit_test.gd` - Already labeled "unit", verify it uses DI
...
- [ ] Run all unit tests: `./scripts/testing/run_tests.sh --individual godot/test/grid_building_test/manipulation/unit/`
- [ ] Run all integration tests: `./scripts/testing/run_tests.sh --individual godot/test/grid_building_test/manipulation/integration/`
- [ ] Run all regression tests: `./scripts/testing/run_tests.sh --individual godot/test/grid_building_test/manipulation/regressions/`
...
**Total suite**: Expect 20-30% faster with new unit tests replacing some integration tests
