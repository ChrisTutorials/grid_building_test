...
func test_targeting_system_with_collision() -> void:
	...
	var offsets: Dictionary = test_env.get_collision_mapper().get_tile_offsets_for_test_collisions(
...
func test_locations_close_to_zero_are_within_tile_map_bounds() -> void:
	...
	assert_int(assigned_rules.size()).is_equal(1).append_failure_message(
		"Expected exactly 1 rule, but found %d rules" % assigned_rules.size()
	)
		...
		for y_offset in range(-test_range, test_range + 1):
			...
			if not validation_result.is_successful():
				...
				for msg: String in unique_messages:
					...
					assert_int(count).is_equal(1).append_failure_message(
						(
							"Duplicate error message '%s' appeared %d times at position %s"
							% [msg, count, test_position]
						)
					)

			# All positions within the test range should be valid
			assert_bool(validation_result.is_successful()).is_true().append_failure_message(
				(
					"Position %s should be within tilemap bounds. Issues: %s"
					% [test_position, str(validation_result.get_issues())]
				)
			)
...
func test_single_position_validation() -> void:
	...
	if not validation_result.is_successful():
		...
		for issue: String in issue_counts:
			...
			if count > 1:
				assert_int(count).is_equal(1).append_failure_message(
					"Duplicate issue '%s' appeared %d times" % [issue, count]
				)

	# This should pass since origin is within a reasonable tilemap
	assert_bool(validation_result.is_successful()).is_true().append_failure_message(
		"Origin position should be valid. Issues: %s" % str(validation_result.get_issues())
	)
