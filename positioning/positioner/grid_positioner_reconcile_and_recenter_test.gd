...
func test_visibility_reconcile_applies_when_differs() -> void:
    ...
    assert_bool(res.apply).append_failure_message("Visibility reconcile should apply when current visibility differs from expected").is_true()
    assert_bool(res.visible).append_failure_message("Visibility reconcile should set visible=true when expected visibility is true").is_true()
    assert_str(res.reason).append_failure_message("Visibility reconcile should provide reason for visibility change").is_equal("reconcile_should_be_visible")
...
func test_recenter_decision_none() -> void:
    ...
    assert_int(d).append_failure_message("NONE policy should always return NONE decision").is_equal(GridPositionerLogic.RecenterDecision.NONE)
...
func test_recenter_decision_last_shown_prefers_cache() -> void:
    ...
    assert_int(d).append_failure_message("LAST_SHOWN policy with valid cache should return LAST_SHOWN decision").is_equal(GridPositionerLogic.RecenterDecision.LAST_SHOWN)
...
func test_recenter_decision_last_shown_fallback_mouse_then_center() -> void:
    ...
    assert_int(d1).append_failure_message("LAST_SHOWN policy without cache should fallback to MOUSE_CURSOR when mouse available").is_equal(GridPositionerLogic.RecenterDecision.MOUSE_CURSOR)
    ...
    assert_int(d2).append_failure_message("LAST_SHOWN policy without cache or mouse should fallback to VIEW_CENTER").is_equal(GridPositionerLogic.RecenterDecision.VIEW_CENTER)
...
func test_recenter_decision_mouse_cursor_prefers_cache_or_viewport() -> void:
    ...
    assert_int(d1).append_failure_message("MOUSE_CURSOR policy should return MOUSE_CURSOR when mouse available").is_equal(GridPositionerLogic.RecenterDecision.MOUSE_CURSOR)
    ...
    assert_int(d2).append_failure_message("MOUSE_CURSOR policy should return MOUSE_CURSOR when mouse available").is_equal(GridPositionerLogic.RecenterDecision.MOUSE_CURSOR)
    ...
    assert_int(d3).append_failure_message("MOUSE_CURSOR policy without mouse should fallback to VIEW_CENTER").is_equal(GridPositionerLogic.RecenterDecision.VIEW_CENTER)
...
func test_recenter_decision_view_center() -> void:
    ...
    assert_int(d).append_failure_message("VIEW_CENTER policy should always return VIEW_CENTER decision").is_equal(GridPositionerLogic.RecenterDecision.VIEW_CENTER)
