...
func test_visibility_reconcile_applies_when_differs() -> void:
    ...
    assert_bool(res.apply)
    	.append_failure_message("Visibility reconcile should apply when current visibility differs from expected")
    	.is_true()
    assert_bool(res.visible)
    	.append_failure_message("Visibility reconcile should set visible=true when expected visibility is true")
    	.is_true()
...
func test_recenter_decision_none() -> void:
    ...
    assert_int(d)
    	.append_failure_message("NONE policy should always return NONE decision")
...
func test_recenter_decision_last_shown_prefers_cache() -> void:
    ...
    assert_int(d)
    	.append_failure_message("LAST_SHOWN policy with valid cache should return LAST_SHOWN decision")
...
func test_recenter_decision_last_shown_fallback_mouse_then_center() -> void:
    ...
    assert_int(d1)
    	.append_failure_message("LAST_SHOWN policy without cache should fallback to MOUSE_CURSOR when mouse available")
    ...
    assert_int(d2)
    	.append_failure_message("LAST_SHOWN policy without cache or mouse should fallback to VIEW_CENTER")
...
func test_recenter_decision_mouse_cursor_prefers_cache_or_viewport() -> void:
    ...
    assert_int(d1)
    	.append_failure_message("MOUSE_CURSOR policy should return MOUSE_CURSOR when mouse available")
    	.is_equal(GridPositionerLogic.RecenterDecision.MOUSE_CURSOR)
