...
func _create_positioner_with_policy(policy: GBEnums.CenteringMode, enable_input: bool) -> GridPositioner2D:
...
func _get_position_diagnostics(positioner: GridPositioner2D, policy_name: String) -> String:
	...
	var viewport_size := env.get_viewport().get_visible_rect().size if env.get_viewport() else Vector2.ZERO
	return "Policy: %s, Position: %s, Visible: %s, In tree: %s, Enabled: %s, Camera: %s, Viewport: %s" % [
		policy_name,
...
func test_center_on_mouse_vs_center_on_screen_policies() -> void:
	...
	var positioner_mouse := _create_positioner_with_policy(GBEnums.CenteringMode.CENTER_ON_MOUSE, false)
	...
	var positioner_screen := _create_positioner_with_policy(GBEnums.CenteringMode.CENTER_ON_SCREEN, false)
	...
	assert_bool(positioner_mouse.visible).append_failure_message(
		"Mouse policy positioner should be visible. %s" % mouse_diag
	).is_true()

	assert_bool(positioner_screen.visible).append_failure_message(
		"Screen policy positioner should be visible. %s" % screen_diag
	).is_true()
	...
	assert_bool(mouse_from_origin > 40.0 and screen_from_origin > 90.0).append_failure_message(
		"Both policies should maintain assigned positions. Mouse dist: %.2f (expected ~70), Screen dist: %.2f (expected ~141), Policy dist: %.2f | Mouse: %s | Screen: %s" % [
			mouse_from_origin, screen_from_origin, position_distance, mouse_diag, screen_diag
		]
	).is_true()
