...
func test_should_be_visible_off_mode_active_when_off() -> void:
	...
	assert_bool(result)
		.append_failure_message("Positioner should be visible in OFF mode when remain_active_in_off_mode is true")
		.is_true()
...
func test_should_be_visible_off_mode_not_active_when_off() -> void:
	...
	var result := GridPositionerLogic.should_be_visible(GBEnums.Mode.OFF, settings, last_mouse, has_mouse)
	assert_bool(result)
		.append_failure_message("Positioner should be hidden in OFF mode when remain_active_in_off_mode is false")
		.is_false()
...
func test_should_be_visible_active_mode_with_last_mouse_allowed() -> void:
	...
	assert_bool(result)
		.append_failure_message("Positioner should be visible in MOVE mode when last_mouse is allowed")
		.is_true()
...
func test_should_be_visible_active_mode_with_mouse_world_and_enabled() -> void:
	...
	var result := GridPositionerLogic.should_be_visible(GBEnums.Mode.MOVE, settings, last_mouse, has_mouse)
	assert_bool(result)
		.append_failure_message("Expected positioner to be visible with cached mouse world. Settings: mouse_enabled=%s, has_mouse_world=%s, last_mouse_allowed=%s, hide_on_handled=%s" % [str(settings.enable_mouse_input), str(has_mouse), str(last_mouse.allowed), str(settings.hide_on_handled)])
		.is_true()
...
func test_should_be_visible_for_mode_off_active() -> void:
	...
	assert_bool(result)
		.append_failure_message("Positioner should be visible for OFF mode when remain_active_in_off_mode is true")
		.is_true()
...
func test_should_be_visible_for_mode_off_not_active() -> void:
	...
	var result := GridPositionerLogic.should_be_visible_for_mode(GBEnums.Mode.OFF, settings)
	assert_bool(result)
		.append_failure_message("Positioner should be hidden for OFF mode when remain_active_in_off_mode is false")
		.is_false()
...
func test_visibility_decision_trace() -> void:
  ...
  .append_failure_message("Visibility trace should contain current mode").contains("mode=MOVE")
	assert_str(trace)
		.append_failure_message("Visibility trace should contain last_mouse_allowed status").contains("last_mouse_allowed=true")
	assert_str(trace)
		.append_failure_message("Visibility trace should contain has_mouse_world status").contains("has_mouse_world=true")
	assert_str(trace)
		.append_failure_message("Visibility trace should contain mouse_enabled status").contains("mouse_enabled=true")
	assert_str(trace)
		.append_failure_message("Visibility trace should contain computed visibility result").contains("computed_should=true")
...
func test_visibility_decision_trace_null_mode_state() -> void:
	...
	assert_str(trace)
		.append_failure_message("Visibility trace should handle null mode_state gracefully").contains("mode=<none>")
...
func test_visibility_decision_trace_null_settings() -> void:
	...
	assert_str(trace)
		.append_failure_message("Visibility trace should handle null settings gracefully").contains("mouse_enabled=<n/a>")
