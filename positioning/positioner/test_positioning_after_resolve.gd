...
func test_positioner_moves_after_resolve_dependencies() -> void:
	...
	var pre_resolve_diag := (
		"Pre-resolve: pos=%s, visible=%s, in_tree=%s"
		% [
	...
	var post_resolve_diag := (
		"Post-resolve: pos=%s, visible=%s, distance=%.2f"
		% [str(final_position), str(positioner.visible), distance_from_origin]
	)
