...
func test_mouse_event_noop_when_hide_on_handled_false() -> void:
	...
	assert_bool(res.apply).append_failure_message("When hide_on_handled=false, mouse events should not apply visibility changes").is_false()
...
func test_hide_on_handled_applies_when_mouse_enabled() -> void:
	var settings := _make_settings(true, true, true)  # active_when_off=true, hide_on_handled=true, mouse_enabled=true
	var blocked_mouse_status := GBMouseInputStatus.new()
	blocked_mouse_status.set_from_values(false, Vector2.ZERO, 0, "blocked", Vector2.ZERO)  # blocked input
