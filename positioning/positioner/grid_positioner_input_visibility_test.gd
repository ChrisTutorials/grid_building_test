...
func test_off_mode_active_should_be_visible() -> void:
  ...
  .append_failure_message("OFF mode with active_when_off=true should be visible").is_true()
	assert_bool(GridPositionerLogic.should_be_visible_for_mode(GBEnums.Mode.OFF, s))
		.append_failure_message("OFF mode with active_when_off=true should be visible for mode only")
		.is_true()
...
func test_off_mode_inactive_should_be_hidden() -> void:
  ...
  .append_failure_message("OFF mode with active_when_off=false should be hidden").is_false()
	assert_bool(GridPositionerLogic.should_be_visible_for_mode(GBEnums.Mode.OFF, s))
		.append_failure_message("OFF mode with active_when_off=false should be hidden for mode only")
		.is_false()
...
func test_mouse_event_noop_when_hide_on_handled_false() -> void:
	...
	assert_bool(res.apply)
		.append_failure_message("When hide_on_handled=false, mouse events should not apply visibility changes")
		.is_false()
...
func test_hide_on_handled_ignored_when_mouse_disabled() -> void:
 ...
 assert_bool(result)
 	...
 	.is_false()
 	.append_failure_message( "When mouse input is enabled, hide_on_handled should still apply with blocked mouse input status. " + "Settings: hide_on_handled=%s, mouse_enabled=%s, mouse_status.allowed=%s" % [str(settings.hide_on_handled), str(settings.enable_mouse_input), str(blocked_mouse_status.allowed)] )
 	.is_true()