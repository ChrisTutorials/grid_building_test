...
func test_environment_has_no_issues() -> void:
	...
	assert_array(issues).is_empty().append_failure_message("CollisionTestEnvironment should have no issues: " + str(issues))
...
func test_container_access() -> void:
	var container: GBCompositionContainer = test_env.get_container()
	assert_that(container).is_not_null().append_failure_message("Composition container should be accessible")
