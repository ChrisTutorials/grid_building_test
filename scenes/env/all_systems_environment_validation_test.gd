...
func test_core_systems_present() -> void:
	...
	assert_that(test_env.level_context).is_not_null().append_failure_message("GBLevelContext should be present")
	assert_that(test_env.tile_map_layer).is_not_null().append_failure_message("TileMapLayer should be present")
...
func test_building_systems_present() -> void:
	...
	assert_that(test_env.manipulation_parent).is_not_null().append_failure_message("ManipulationParent should be present")
	assert_that(test_env.indicator_manager).is_not_null().append_failure_message("IndicatorManager should be present")

## Test: All systems specific to AllSystemsTestEnvironment are present
func test_all_systems_specific_present() -> void:
	assert_that(test_env.manipulation_system).is_not_null().append_failure_message("ManipulationSystem should be present")
	assert_that(test_env.target_highlighter).is_not_null().append_failure_message("TargetHighlighter should be present")
...
func test_grid_targeting_system_positioner_connection() -> void:
	...
	assert_that(test_env.positioner).is_not_null().append_failure_message("Positioner should be available in environment")
	...
	assert_that(test_env.grid_targeting_system).is_not_null().append_failure_message("GridTargetingSystem should be properly initialized")
...
func test_indicator_manager_registration() -> void:
	...
	assert_array(building_issues).is_empty().append_failure_message("BuildingSystem should have no runtime issues: " + str(building_issues))
...
func test_level_context_validation() -> void:
	...
	assert_array(level_issues).is_empty().append_failure_message("LevelContext should have no runtime issues: " + str(level_issues))
...
func test_environment_no_issues() -> void:
	...
	assert_array(all_issues).is_empty().append_failure_message("Environment should have no issues: " + str(all_issues))
...
func test_building_system_can_enter_build_mode() -> void:
	...
	if placeable != null:
		...
		assert_that(result.is_successful()).is_true().append_failure_message("BuildingSystem should be able to enter build mode with test placeable")
	else:
		# Factory couldn't create placeable, just test that the building system exists
		assert_that(test_env.building_system).is_not_null().append_failure_message("BuildingSystem should exist even when placeable creation fails")
...
func test_scene_node_structure() -> void:
	...
	assert_that(test_env.get_node_or_null("World")).is_not_null().append_failure_message("World node should exist at expected path")
	assert_that(test_env.get_node_or_null("World/GridPositioner2D")).is_not_null().append_failure_message("GridPositioner2D should exist at expected path")
...
func test_placement_rules_available() -> void:
	...
	if container != null:
		...
		assert_that(placement_rules).is_not_null().append_failure_message("Placement rules collection should exist")
