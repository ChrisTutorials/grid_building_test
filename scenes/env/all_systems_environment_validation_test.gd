...
func test_core_systems_present() -> void:
	...
	assert_that(test_env.positioner).append_failure_message(
		"GridPositioner2D should be present"
		).is_not_null()
...
func test_all_systems_specific_present() -> void:
	assert_that(test_env.manipulation_system).append_failure_message(
		"ManipulationSystem should be present").is_not_null()
	assert_that(test_env.target_highlighter).append_failure_message(
		"TargetHighlighter should be present").is_not_null()
...
func test_grid_targeting_system_positioner_connection() -> void:
	...
	assert_array(targeting_issues) \
		.append_failure_message("GridTargetingSystem should have no runtime issues: " + str(targeting_issues)).is_empty()
	...
	assert_that(test_env.positioner).append_failure_message("Positioner should be available in environment").is_not_null()
	...
	assert_that(test_env.grid_targeting_system).append_failure_message(
		"GridTargetingSystem should be properly initialized").is_not_null()
...
func test_indicator_manager_registration() -> void:
	...
	assert_array(building_issues) \
		.append_failure_message("BuildingSystem should have no runtime issues: " + str(building_issues)).is_empty()
...
func test_level_context_validation() -> void:
	...
	assert_array(level_issues) \
		.append_failure_message("LevelContext should have no runtime issues: " + str(level_issues)).is_empty()
...
func test_scene_node_structure() -> void:
	...
	assert_that(test_env.get_node_or_null("World")).is_not_null().append_failure_message(
		"World node should exist at expected path")
	assert_that(test_env.get_node_or_null("World/GridPositioner2D")).is_not_null().append_failure_message(
		"GridPositioner2D should exist at expected path")
	assert_that(test_env.get_node_or_null("World/GridPositioner2D/ManipulationParent")).is_not_null().append_failure_message(
		"ManipulationParent should exist")
	assert_that(test_env.get_node_or_null("World/GridPositioner2D/ManipulationParent/IndicatorManager")).is_not_null().append_failure_message(
		"IndicatorManager should exist at expected path")
...
func test_placement_rules_available() -> void:
	...
	assert_that(placement_rules).append_failure_message(
		"Placement rules collection should exist").is_not_null()
