...
func test_all_systems_specific_present():
...
func test_dependency_injection_setup():
	...
	injector_issues: Node = test_env.injector.get_runtime_issues()
...
func test_grid_targeting_system_positioner_connection():
...
func test_environment_no_issues():
	var all_issues = test_env.get_issues()
...
func test_building_system_can_enter_build_mode():
	...
	if container != null and container.has_method("get_placement_rules"):
		var placement_rules = container.get_placement_rules()
		if placement_rules != null and placement_rules.size() > 0:
			var first_rule = placement_rules[0]
			if first_rule != null and first_rule.has_method("get_placeable"):
				var placeable = first_rule.get_placeable()
				if placeable != null:
					var result = test_env.building_system.enter_build_mode(placeable)
...
func test_placement_rules_available():
