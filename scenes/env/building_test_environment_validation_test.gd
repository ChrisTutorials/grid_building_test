...
func test_environment_has_no_issues() -> void:
	...
	(
		assert_array(issues) \
		. append_failure_message("BuildingTestEnvironment should have no issues: " + str(issues)) \
		. is_empty()
	)
...
func test_indicator_manager_registration() -> void:
	...
	(
		assert_that(test_env.indicator_manager) \
		. append_failure_message("IndicatorManager should be accessible") \
		. is_not_null()
	)
...
func test_scene_node_structure() -> void:
	...
	(
		assert_that(test_env.get_node_or_null("World")) \
...
func test_grid_targeting_system_setup() -> void:
	...
	(
		assert_that(test_env.positioner) \
		. append_failure_message("Positioner should be available for targeting system") \
		. is_not_null()
	)
...
func test_owner_root_access() -> void:
	...
	(
		...
		. append_failure_message("Owner root should be accessible through environment") \
		. is_not_null()
	)
