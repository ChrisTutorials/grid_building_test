...
func test_environment_has_no_issues() -> void:
	...
	assert_array(issues).is_empty().append_failure_message("BuildingTestEnvironment should have no issues: " + str(issues))
...
func test_owner_root_access() -> void:
	...
	assert_that(owner_root).is_not_null().append_failure_message("Owner root should be accessible through environment")
