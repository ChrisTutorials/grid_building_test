...
func _transforms_equal(
	t1: Transform2D, t2: Transform2D, tolerance: float = ROTATION_TOLERANCE
) -> bool:
	"""Compare two Transform2D objects with floating point tolerance"""
	return (
		abs(t1.get_rotation() - t2.get_rotation()) < tolerance
		and t1.get_origin().is_equal_approx(t2.get_origin())
		and t1.get_scale().is_equal_approx(t2.get_scale())
		and abs(t1.get_skew() - t2.get_skew()) < tolerance
	)
...
func _angles_equivalent(
	angle1: float, angle2: float, tolerance: float = ROTATION_TOLERANCE
) -> bool:
...
func _calculate_expected_rotation_delta(
	initial_rotations: Dictionary, applied_degrees: float
) -> Dictionary:
	...
	return {
		...
		"manipulation_parent":
		_normalize_angle(initial_rotations.manipulation_parent + applied_degrees),
		"indicator_manager":
		_normalize_angle(initial_rotations.indicator_manager + applied_degrees),
...
func test_isometric_rotation_scenarios(
	...
	var expected_rotations: Dictionary = _calculate_expected_rotation_delta(
		initial_rotations, rotation_increment
	)
	...
	(
		...
		. append_failure_message(
			(
				...
				% [
					scenario_name,
					expected_rotations.indicator_manager,
...
func test_isometric_cumulative_rotation(
	...
	var expected_final_rotation: float = _normalize_angle(
		initial_rotations.manipulation_parent + total_rotation
	)
	...
	for i in range(indicators.size()):
		var expected_indicator_rotation: float = _normalize_angle(
			initial_rotations.indicators[i] + total_rotation
		)
		var actual_indicator_rotation: float = final_rotations.indicators[i]
...
func test_isometric_transform_isolation(
	...
	var base_global_rotation_after_manipulation: float = (
		_base_transform_node.global_rotation_degrees
	)
...
static func _generate_cumulative_test_parameters() -> Array:
	...
	var cumulative_scenarios: Array = [
		"identity_baseline", "standard_isometric", "45_degree_isometric"
	]
...
func test_grid_aware_rotation_fix_in_isometric_context(
	...
	for i in range(4):
		...
		GBGridRotationUtils.set_node_direction(
			_manipulation_parent, target_direction, tile_map, false
		)
...
static func _generate_isolation_test_parameters() -> Array:
	...
	for scenario in TEST_SCENARIOS:
		parameters.append(
			[scenario.name, scenario.base_rotation_deg, scenario.base_skew_deg, scenario.base_scale]
		)
